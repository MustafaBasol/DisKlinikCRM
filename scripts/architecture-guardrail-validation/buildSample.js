...
for (const [, arr] of groupList) arr.sort((a, b) => (a.id < b.id ? -1 : a.id > b.id ? 1 : 0));
...
for (const dom of ownerDomains) {
  ...
  if (!already) {
    const f = findings.filter((x) => x.ownerDomain === dom).sort((a, b) => (a.id < b.id ? -1 : 1))[0];
...
for (const dom of callerDomains) {
  ...
  if (!already) {
    const f = findings.filter((x) => x.callerDomain === dom).sort((a, b) => (a.id < b.id ? -1 : 1))[0];
...
const finalList = [...selected.values()].sort((a, b) => (a.finding.id < b.finding.id ? -1 : 1));

const manifest = {
  schemaVersion: '1.0.0',
  taskId: 'F2-GUARDRAIL-VAL-001',
  sourceReport: 'docs/program/evidence/tooling/F2-GUARDRAIL-VAL-001_raw_scan_report.json',
  sourceReportRepositorySha: r.repositorySha,
...
console.log(`Sample size: ${finalList.length} (required minimum: 120)`);
