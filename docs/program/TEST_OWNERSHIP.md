...
- **3 yeni, tamamen eklemeli (`server:test`'i değiştirmeyen) aggregate script** eklendi: `server:test:non-disposable` (68 üye, çalıştırıldı, çıkış kodu 1, tek bilinen hata `test:overdue-installments`'ın önceden belgelenmiş 2-assertion drift'i, zincirin sonuna kasıtlı olarak yerleştirildi), `server:test:disposable-db` (9 üye, bu ortamda çalıştırılamadı — disposable Postgres yok), `server:test:storage-integration` (1 üye, bu ortamda çalıştırılamadı — MinIO yok).
