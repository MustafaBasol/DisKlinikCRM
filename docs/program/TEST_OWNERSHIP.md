...
## 10.1 F1-003-P1 güncellemesi (2026-07-28) — script kapatma ve execution contract

§6'daki 8 script'siz dosya ve 17 zincir-dışı script bulguları **F1-003-P1** tarafından kapatıldı (bu bölümün geri kalanı, F1-002'nin kendi taban çizgisi olarak, değiştirilmeden korunmuştur — aşağıdaki yalnızca bir güncelleme notudur, geriye dönük bir yeniden yazım değildir):

- **8 script'siz dosyanın tamamına** `server/package.json`'da özel bir `test:*` leaf script eklendi (`test:channel-consent-gate`, `test:clinic-legal-profile`, `test:patient-shared-phone`, `test:platform-backup`, `test:treatment-package-permissions`, `test:kvkk-high006-batch2`, `test:meta-whatsapp-post-booking`, `test:plan-limits-target-clinic-fix`). Script'siz dosya sayısı artık **0**.
- **17 zincir-dışı script**, gerekli terminolojiyle (`accidental_gap` / `intentional_runtime_separated` / `duplicate_alias` / `unsafe_until_f1-003-p2`) yeniden sınıflandırıldı: DB-gerektiren grup **10**'a düzeltildi (§6'daki "+10" zaten doğruydu, ancak F1-002-P1'in kendi MD §4.1 metni `test:platform-admin-password-recovery`'yi yanlışlıkla "no apparent reason" grubuna koymuştu — kendi JSON'ı zaten doğruydu, bu yalnızca metin düzeltmesidir); "no apparent reason" grubu **7**'ye düzeltildi (§6'daki "+1 istisna" `test:file-backup` dahil, artık 7 üyeli).
- **3 yeni, tamamen eklemeli (`server:test`'i değiştirmeyen) aggregate script** eklendi: `server:test:non-disposable` (68 üye, çalıştırıldı, çıkış kodu 1, tek bilinen hata `test:overdue-installments`'ın önceden belgelenmiş 2-assertion drift'i, zincirin sonuna kasıtlı olarak yerleştirildi — **[F1-003-R1B güncellemesi, 2026-07-29]: bu, P1'in kendi tarihsel sonucudur, korunmuştur; sonrasında F1-003-B1 (PR #258, `MERGED`, merge commit `bb4186793323485c71b91ad6939a0d8469f886cd`) `overdueInstallments.test.ts`'i düzeltti — güncel durum: çıkış kodu `0`, `test:overdue-installments` 9/9**), `server:test:disposable-db` (9 üye, bu ortamda çalıştırılamadı — disposable Postgres yok, F1-003-P2 henüz başlamadı), `server:test:storage-integration` (1 üye, bu ortamda çalıştırılamadı — MinIO yok, F1-003-P2 henüz başlamadı).
- `messagesConsentGate.test.ts`'in runtime sınıflandırması `UNKNOWN`'dan `DISPOSABLE_POSTGRES + EXTERNAL_PROVIDER_MOCK` (confidence `HIGH`) durumuna çözüldü; script'i zaten mevcuttu ve zaten `server:test` zincirindeydi.
- **Yeni bulgu:** legacy `server:test` kendisi hiç değiştirilmedi, ancak zaten sessizce **23** DB-gerektiren script (§6'nın "3 örnek" iması yerine tam liste) içerdiği tespit edildi — F1-003-P2'ye açık, çözülmemiş bir bulgu olarak devredildi. **F1-003-R1A düzeltmesi (2026-07-28):** F1-003-P1'in bu bulguya eklediği "+ 1 ek MinIO-gerektiren script (`test:kvkk-lifecycle`)" iddiası nesnel bir olgu hatasıydı — `test:kvkk-lifecycle`'ın S3 sağlayıcısı testin geçtiği her yolda mock'lanmıştır, canlı bir MinIO/S3 uç noktasına hiç erişilmez. Legacy `server:test` PostgreSQL-gerektiren ama MinIO-gerektirmeyen bir agregatördür. Programdaki tek gerçek MinIO-bağımlı hedef `test:file-backup-db-integration`'dır (legacy `server:test`'in üyesi değildir). Detay: [evidence/F1-003-P1_TEST_SCRIPT_CLOSURE_AND_EXECUTION_CONTRACT.md](evidence/F1-003-P1_TEST_SCRIPT_CLOSURE_AND_EXECUTION_CONTRACT.md) §18.

Tam detay: [evidence/F1-003-P1_test_execution_contract.json](evidence/F1-003-P1_test_execution_contract.json) (114 kayıt, F1-003-R1A tarafından `test:kvkk-lifecycle`/legacy `server:test` MinIO alanları düzeltildi), [evidence/F1-003-P1_TEST_SCRIPT_CLOSURE_AND_EXECUTION_CONTRACT.md](evidence/F1-003-P1_TEST_SCRIPT_CLOSURE_AND_EXECUTION_CONTRACT.md) (§18'de F1-003-R1A düzeltme özeti).
