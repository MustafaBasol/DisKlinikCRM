...
Son güncelleme: 2026-08-22 (F5-2 — program sahibi kararı: ADR-006/ADR-007 `NEEDS_POC` → `ACCEPTED_WITH_CONDITIONS`, F5-1P disposable PoC kanıtı üzerine. Önceki tur: F0-010 — Queue and Transactional Outbox PoC Design, ADR-006/ADR-007 `DEFERRED` → `NEEDS_POC`)
...
- **Status:** `ACCEPTED_WITH_CONDITIONS` (program-owner decision, 2026-08-22, on F5-1P PoC evidence) — the narrow principle "events must be versioned and consumers idempotent if/when an outbox is built" remains an already-binding invariant independent of this ADR's own status
- **Program-owner decision (2026-08-22):** The transactional outbox is **accepted for the current stage**, with one binding condition: it is authorized **ONLY for genuinely outbox-shaped flows** — committed database state plus a required side effect/event that must not be lost. It is explicitly **NOT** authorization to migrate every cron/job onto an event bus. F5-2 implements against this decision: `OutboxEvent`/`OutboxConsumerExecution`, a versioned event contract registry, a transaction-only producer, a `SKIP LOCKED` dispatcher with leases/bounded retry/dead-letter, audited replay, a business idempotency ledger, and **one** integrated flow selected on repository evidence (the appointment-request confirmation's commit→side-effect gap; the other three F0-010 candidates were inspected and rejected as schedule-shaped or same-database writes). Nothing is merged, deployed or activated: both cutover flags default OFF. Evidence: [evidence/F5-2_TRANSACTIONAL_OUTBOX_AND_EVENT_REGISTRY.md](evidence/F5-2_TRANSACTIONAL_OUTBOX_AND_EVENT_REGISTRY.md). **Still open:** real volume projections (require production observability — ADR-012 `DEFERRED`), and outbox/idempotency-table retention (not yet in `dataRetentionCleanupJob`), which is a rollout blocker.
...
- **Status:** `ACCEPTED_WITH_CONDITIONS` (program-owner decision, 2026-08-22, on F5-1P PoC evidence)
- **Program-owner decision (2026-08-22):** The current-stage mechanism is a **PostgreSQL transactional outbox plus an in-process dispatcher**. **BullMQ/Redis is DEFERRED** — not rejected — and may be reconsidered only after a MEASURED trigger: dispatcher/claim contention, insufficient throughput, significant delayed/repeat scheduling requirements, multi-consumer fan-out, or another demonstrated operational/scaling trigger. No BullMQ runtime dependency may be introduced meanwhile (F5-2 asserts this structurally: `tests/outboxContracts.test.ts` fails if any outbox runtime module imports `bullmq` or `ioredis`). This follows F5-1P's `ADR_007_RECOMMENDATION = B`: commit-and-publish atomicity is the one gap that actually loses events and Redis cannot enlist in a PostgreSQL transaction (E11c); throughput was comparable once measured symmetrically (1,887/s vs 1,684/s @ n=1000); and BullMQ would promote an optional, fail-open Redis to a business-critical second failure domain on a single-VPS topology. F5-2 additionally ships per-organization backlog metrics **specifically so the deferral trigger is measurable** rather than nominal.
