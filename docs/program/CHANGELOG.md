...
- **Tests:** `test:storage-key-contract` 70/70 (was 41), `test:lab-orders` 35/35 (was 32), `test:clinic-bulk-export` 117/117, `test:imaging` 103/103, `test:patient-privacy` 38/38, `test:kvkk-lifecycle` 113/113, `test:lab-attachment-legal-hold` 21/21, `test:storage-deletion-evidence` 34/34, `test:file-preview` 12/12, `typecheck` exit 0; root `guardrail:scan` exit 0, `log-privacy-guard:scan --strict-baseline` no new violations, `git diff --check` exit 0. The `server:test:non-disposable` aggregate exits 1 on **one pre-existing failure** in `platformBackup.test.ts` (`clinic-type token rejected with 403`) — **proven pre-existing** by reproducing it identically on the stashed clean baseline, not merely asserted; unrelated to storage keys.
- **CI classification:** `runBackendGeneral`/`runPostgres`/`runStorage` = true, `runLegacyBackend` = false. **Reported gap:** `test:clinic-bulk-export` belongs only to `server:test:legacy-db-required`, so this PR's `clinicBulkExport.test.ts` edit runs in **no** CI layer (local 117/117 is a local fact, not a CI fact). CI wiring **not** changed — out of scope.
