...
`F4-3_STATUS = AGENT_COMPLETED` · `NOT_MERGED` / `NOT_DEPLOYED` / `NOT_PRODUCTION_VERIFIED`
...
veya worker icat etmez.** Kabul edilmiş sözleşmeleri kullanır: kanıt defteri
`writeAuditLog`, operatör sinyali `recordOperationalEvent`, idempotency
semantiği ise dışa aktarma artefaktları için zaten kabul edilmiş
`deleteStorageObjectIdempotent` deseninin aynısı.
...
| Kanıt | Yalnızca engellenen (legal-hold) dalda `AuditLog`; başarılı silmede **hiç yok** | Her sonuç (retler dâhil) `AuditLog` yazar: klinik, organizasyon, entity tipi/id, aksiyon, `requestedAt`, `executedAt`, sonuç, hata kodu, aktör |
| Yükseltme (escalation) | Lab siparişlerinde **tamamen sessiz** (`catch(() => {})`); eklerde yalnızca `console.error` | Terminal olmayan her sonuç ayrıca `severity=error` `OperationalEvent` yazar — DB satırı gittikten sonra nesneyi **hâlâ adlandıran** tek artefakt |
...
| `server/src/routes/attachments.ts` | DELETE ve yükleme geri-alma yolları sözleşmeye bağlandı; yanıt `storageDeletion` sonucunu taşır |
| `server/src/routes/labOrders.ts` | Aynısı + DB silmesi sahibine kapsamlandı (`deleteMany`) |
| `server/src/tests/storageDeletionEvidence.test.ts` **(yeni)** | 23 iddia; davranışsal + yapısal |
...
```
npm run test:storage-deletion-evidence  -> exit 0 ·  23 passed / 0 failed
