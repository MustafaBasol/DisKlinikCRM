...
**(Eklendi 2026-08-22, F4-ATTACH-001-R1-REVIEW-FIX — aynı PR #484 üzerinde altı mimari-inceleme/CI blokajı düzeltildi; exact-head CI artık tamamen YEŞİL (`df92fbd`). Hâlâ `DRAFT`, hâlâ ready/merge/deploy yok — bu karar bilinçli olarak bu görevin kapsamı dışında.)** Çalıştırılabilir bit CI hatası (repodaki gerçek konvansiyona göre `bash "$script"` çağrısıyla düzeltildi, script'lerin git modu `100644` kalır — bkz. `noramedi-opscheck.test.sh`'in kendi yorumu), "append-only" iddiası "NOT enforced append-only" olarak düzeltildi, backup/check/restore-proof'un paylaşılan durum dosyası için AYRI, kısa ömürlü bir durum-yazma kilidi + atomik yazma eklendi (restore-proof artık kendi işlem kilidine de sahip), SSH anahtarı `/etc/noramedi/attachment-vps2/` altına (ev dizini yerine) taşındı ve `ProtectHome=yes` ile tutarlı hale getirildi, `/var/lib/noramedi` paylaşılan dizinine dokunan kurulum komutu kaldırıldı (yerine özel `/var/lib/noramedi-attachment-vps2/status/`), ve SFTP chroot/yol tutarlılığı kanıt belgesi §4a'da tam olarak belgelendi (tedarik edilmeden). **Ayrıca, exact-head CI'ı yeşile taşırken dört ek gerçek kusur bulundu ve düzeltildi** — orijinal PR'ın kendi testinde hiç görünmemişlerdi çünkü her runtime-kilit testi `flock` olmayan bir host'ta yapısal olarak atlanıyor: (1) "flock yok" testinde usrmerge nedeniyle `flock`'un `/bin` üzerinden de erişilebilir kalması, (2) `exec N>DOSYA`'nın betiğin kendi stderr'ini kalıcı olarak `/dev/null`'a yönlendirmesi, ardından bare bir subshell ifadesinin `set -e` altında kendi çıkış kodunu hiç yakalayamadan tüm betiği durdurması — ikisi de flock'un kendi `flock DOSYA KOMUT` biçimiyle değiştirilerek kökünden çözüldü, (3) `[[ ]] && fail` bare ifadesinin `set -e` altında başarı yolunda "başarısız komut" sayılması, ve (4) — asıl kök neden — üç `node -e` betiğinin `process.argv`'yi bir script-yolu yer tutucusu varmış gibi iki konum atlayarak destructure etmesi (`node -e` için böyle bir yer tutucu yok; pozisyonel argümanlar `argv[1]`'den başlar), bu da her argümanın bir konum erken okunmasına ve gerçek bir `flock` bu yolu ilk kez tamamına kadar çalıştırdığında yakalanmamış bir `ENOENT` ile çökmesine yol açıyordu. Tam ayrıntı: master tracker madde 34, kanıt belgesi §3.1/§3.2/§4a/§4b/§5/§8.1.
