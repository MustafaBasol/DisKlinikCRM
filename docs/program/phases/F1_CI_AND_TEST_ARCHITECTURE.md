...
| Phase | F1 — CI and Test Architecture |
| Status | `AGENT_COMPLETED` (2026-07-28 — refreshed inventory/ownership/script-reconciliation delivered; updated 2026-07-28 by F1-002-R1 to integrate F1-002-P1/P2 parallel evidence and reconcile against `origin/main`; PR #253 open, not merged; see [F1-002_TEST_INVENTORY_REFRESH_AND_OWNERSHIP_RECONCILIATION.md](../evidence/F1-002_TEST_INVENTORY_REFRESH_AND_OWNERSHIP_RECONCILIATION.md).) |
...
| Next-task dependency | The exact next F1 CI-implementation task draws on backlog items 2 ("PR / main / nightly / release CI katmanlarının kurulumu") and 3 ("Test veritabanı stratejisi (disposable Postgres standardı)") together, not item 2 alone — F1-002-P2's findings show CI-layer setup cannot be usefully sequenced ahead of a committed disposable-Postgres/MinIO provisioning mechanism, since 22+1 of 137 current targets cannot run in any CI today without one. At minimum that future task must address: the 8 unscripted backend test files and the 17 scripts excluded from the `server:test` aggregate chain (§0.1 below and `TEST_OWNERSHIP.md` §6); authoritative CI command coverage (only 7 of 116 JS/TS test files reached by any workflow today, per F1-002-P1); a disposable-Postgres provisioning design/implementation boundary (no committed script/Compose file exists — F1-002-P2 §8); the new MinIO/S3-emulator test dependency (`fileBackupDbIntegration.test.ts`, zero provisioning exists); safe parallel-worktree isolation (no port/database-name collision-avoidance mechanism exists today); and a default full-suite fail-safe for anything not yet classified. This task remains unblocked on `MERGED` status for F1-001/F1-002-P1/F1-002-P2 (all now `MERGED`); it remains blocked on F1-002 main (PR #253) merging, and on external decision of its exact task ID — not invented here, not started by this integration task. |
