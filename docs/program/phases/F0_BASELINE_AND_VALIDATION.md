...
Faz durumu: `IN_PROGRESS` · Son güncelleme: 2026-07-19 (F0-007 düzeltme turu — [PR #174](https://github.com/MustafaBasol/DisKlinikCRM/pull/174)'ün dış incelemesi, birincil ağacın dal/HEAD'inin "değişmedi" iddiasını ve `--limit 60`'a dayalı yanlış PR-tarama iddiasını düzeltti — bkz. backlog F0-007 satırı, R-053. Önceki tur: F0-007 tamamlandı — KVKK aktif çalışma taban çizgisi ve mimari dondurma sınırı; F0-006'nın kendi `MERGED` durumu düzeltildi — bkz. PR #173; PR #167'nin bayat `OPEN` kaydı düzeltildi)
...
| F0-007 | Active KVKK Work Baseline and Architecture Freeze Boundary | `PR_OPEN` — [PR #174](https://github.com/MustafaBasol/DisKlinikCRM/pull/174) open, under external review; a correction pass was requested and applied (see below). GitHub verification of the named KVKK/privacy/security PR subset (§3.2 of the evidence doc) found zero open among that set. Key finding: KVKK-HIGH-007's base feature (PR #169) is already `MERGED` and deployed (merge commit equals confirmed production HEAD) — the active work in the primary tree is a continuation/hardening phase of that same initiative, not a new task. Corrected a stale "PR #167 OPEN" claim to `MERGED`. **Correction pass:** the original "repository-wide, 61/61 `MERGED`, zero open" claim was generated from a 60-row-capped, non-repository-wide query and was itself corrected — a wider sweep found 3 open PRs (#48, pre-existing/unrelated; #174, this task's own PR; #175, the KVKK-HIGH-007 continuation, opened after this task's original verification, content not inspected); the primary tree's branch/HEAD "unchanged throughout" claim was also corrected to a three-observation, time-sliced record. Deliverables: [KVKK_ACTIVE_WORK_BASELINE.md](../KVKK_ACTIVE_WORK_BASELINE.md), [KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md](../KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md), [evidence/F0-007_KVKK_BASELINE_EVIDENCE.md](../evidence/F0-007_KVKK_BASELINE_EVIDENCE.md), [evidence/F0-007_kvkk_work_inventory.json](../evidence/F0-007_kvkk_work_inventory.json). 13 risks (R-041…R-053) recorded in [RISK_REGISTER.md](../RISK_REGISTER.md). |
