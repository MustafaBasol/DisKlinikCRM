...
**Detection.** `GET /api/health` returns anything other than `200`/`204`/`401`/`403` on repeated probes (both the public and local URL), or a user/clinic report of total unreachability. **F3-OBS-002 status (updated 2026-08-13, F3-OBS-002-CLOSE): `ACTIVE`.** External uptime monitors now poll `/api/livez` and `/api/readyz` every 5 minutes and email the operator on incident and on recovery (§4.11) — **drill-proven end-to-end, DOWN in 61 s with real human email delivery** (evidence doc §18.1). Automated detection of total unreachability therefore exists; manual polling and user reports remain valid corroboration, not the only path. **Superseded and preserved:** this line previously read *"No automated alerting exists yet (ADR-012 `DEFERRED`) — this is a manual polling / user-report detection until that gap closes … an external uptime prober … is proposed but not yet activated in production."* ADR-012 itself remains `DEFERRED` for the wider observability toolset (error tracking, APM, log aggregation, tracing) — availability alerting is the part that is now live, and the rest is not.
...
**Added by F3-OBS-002, not F3-IR-001** — this subsection documents the automated monitoring implemented by [F3-OBS-002](../evidence/F3-OBS-002_LIVE_OBSERVABILITY_WIRING_ALERT_VERIFICATION.md); it does not change this runbook's own F3-IR-001 status header above.

> **`ACTIVE IN PRODUCTION` since 2026-08-13** (F3-OBS-002-CLOSE). **Superseded and preserved as history:** this note previously read *"Nothing in this subsection is active in production yet — every 'Detection' line above that references it (4.1, 4.2, 4.7) still describes manual/human detection as the current reality until the external activation steps below are executed and evidenced."* That was accurate at its own point in time. The external providers are now wired, the systemd timer is `enabled`/`active` on a 5-minute cadence, and **both alert chains have been proven end-to-end to a real human recipient by controlled drill** — detection, delivery, and recovery (evidence doc §17–§18). The detection lines in §4.1, §4.2 and §4.7 are updated accordingly. Alerting is now an **additional** detection path, not a replacement for the manual checks below — those remain valid and are still the right first move when you are already at a terminal.
...
| `noramedi-api` / `noramedi-worker` PM2 status | Host-local: `scripts/noramedi-opscheck.sh` `pm2` check (systemd timer, independent of the two monitored processes) → dead-man's-switch ping | Either process not `online`, or no ping arrives within Period + Grace (**≈20 minutes**, see the note below) |
...
**Live configuration and what the timings actually mean.** The three external HTTP monitors run at a **5-minute** interval. The three dead-man checks are configured **Period = 5 minutes, Grace = 15 minutes**.

> **Read the dead-man timing correctly — this is a documented misreading.** Period 5 m + Grace 15 m means a check goes DOWN after **approximately 20 minutes** of silence measured from its **last successful ping** — *not* 15 minutes, and not a 15-minute detection SLA. The 15-minute figure is the grace component alone, and the `ops/systemd/noramedi-opscheck.timer` comment recommending "grace ≥ 3× the 5-minute interval (~15 minutes)" is about sizing that component, not about total time to alert. Measured in the live drill: **`20 m 02 s`** from last ping to the human's email, with detection bounded to `[19 m 05 s, 20 m 02 s]` (evidence doc §18.2). When triaging, anchor on "last successful ping", not on when you noticed.

> **Timezone hazard when reading the Healthchecks console.** The provider UI was observed rendering timestamps at **UTC+02:00**, while the production host's journal renders at **UTC+03:00**. A console reading and a `journalctl` line for the *same* event therefore differ by an hour, and neither is UTC. This was caught during evidence capture only by cross-checking a console reading against a journal ping; taken at face value it would have skewed every measured latency by two hours. **Convert explicitly at the moment you read a timestamp, not later when reconstructing a timeline** — and prefer the host journal as the authoritative clock. Setting the provider account to UTC is the standing recommendation (evidence doc §20).

**A dead-man check going DOWN means the *reporting* stopped, which is not the same as the *thing* being broken.** The check fires when a ping fails to arrive — which happens if the monitored condition genuinely failed, but equally if the host lost outbound network, the timer was disabled, the script broke, or the ping URL became invalid. Confirm the underlying condition directly (`pm2 jlist`, `df -P`, the backup directory) before declaring the corresponding incident. The inverse is also true and is why the `pm2` check exists at all: an alert that never arrives is itself a signal.
...
**If a deploy refuses with `has uncommitted changes`,** the checkout genuinely has a change that the recorded release SHA would not describe — a modified tracked file or an untracked file — and the refusal is correct. **It is not the deploy's own leftovers.** `dist.next`, `dist.next.stale-<UTC>`, `dist.rollback-<tag>-<UTC>` and `.noramedi-frontend-release-state` are all ignored by the repository's `.gitignore` (F3-PROD-004-R3), so a successful deploy no longer dirties its own checkout and cannot block the next one. Run `git -C /var/www/noramedi status --porcelain` and read what it actually names. **Commit, stash or remove that change; do not reach for `--allow-dirty`, and do not delete a retained `dist.rollback-*` bundle to make the message go away** — those bundles are the rollback path, they are never auto-deleted, and they are not what the gate is complaining about.
...
**Do not read the HTTP status codes as the answer** (F3-PROD-004-R2). Production's `GET /` is a **302** to `/login`, so `verify` follows redirects and accepts any final 2xx; and the SPA fallback answers **200 with `index.html` for any path that is not on disk**, so a `200` on `/release.json` proves nothing by itself.

**A body on its own is not the answer either** (F3-PROD-004-R2-R1). The marker counts only when *all* of the following hold: the marker request's **own final response is a 2xx**, it came from an **acceptable effective origin** (same host, port and scheme as the base URL — an `http` → `https` upgrade of the same host is the one permitted move), and its **parsed body** carries a valid `releaseSha` equal to the promoted bundle's. An error page is not a served file even when its body looks right, and a redirect off this host answers a question about some other host. Both are reported `NOT_SERVED`, and their bodies are not read at all.

| Line | Meaning |
|---|---|
| `PUBLIC_ROOT_URL = <url>` | where `GET /` finally landed. Record it; an unfamiliar destination is a finding, not a detail |
| `PUBLIC_MARKER_STATUS = <code>` | the marker request's own final status. Anything but a 2xx means no marker was served, whatever the body said |
| `PUBLIC_MARKER_URL = <url>` | where the marker request finally landed. If this is not your host, the run proves nothing about your host |
| `PUBLIC_RELEASE_SHA = <sha>` | a real marker is being served, and this is the release in it |
| `PUBLIC_RELEASE_SHA = NOT_SERVED` | no marker was served: the response was not a 2xx, or it came from another origin, or it carried no `releaseSha` — you are looking at the SPA fallback, an error page, or somebody else's host, **not** at a marker |
