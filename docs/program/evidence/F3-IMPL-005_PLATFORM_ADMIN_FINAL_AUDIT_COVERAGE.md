...
## 11. R1 — sensitive logging policy fix on the class-5 best-effort catch paths (2026-08-11)

Architecture review found the PR mergeable-but-blocked: the 4 `.catch((auditErr) => console.error(...))` sites introduced by §3.2 (the 2 file-backup routes' completed/failed audit-write-failure branches) logged the raw `auditErr` object, conflicting with the F3-IMPL-004/F3-IMPL-006 sensitive-logging policy (raw `Error` objects/`.message` must not reach production logs). **Reconciliation:** `git fetch origin --prune` confirmed PR #364 (`F3-IMPL-006`, wave 2) had **not yet merged** to `origin/main` (still on `feature/f3-impl-006-runtime-log-hygiene-wave2`); `origin/main` had advanced to `efebe19` (PR #362, `F3-SEC-EXIT-001`) with zero file overlap against this PR's changed files (`platformAdmin.ts`, `safeError.ts`, `platformAdmin.test.ts`, `platformFileBackupAudit.test.ts`) — confirmed via `git diff <merge-base>..origin/main --stat` on those exact paths, empty diff — so no merge was performed, per this task's own instruction to fix independently and reconcile again only if needed. `safeErrorFields()` (`server/src/utils/safeError.ts`) already existed on `origin/main` (added by F3-IMPL-004, merged via PR #356) — no new helper was written.

**Fix:** all 4 sites now log `safeErrorFields(auditErr)` instead of the raw `auditErr`, matching the pattern already used by every other fixed call site in this codebase (`errorName`/`errorCode` only — no message, no stack, no DB/provider detail). Two pre-existing, structurally-identical `[platform-backup]` (DB-backup, not file-backup) raw-`auditErr` log sites and one pre-existing `auditMsg` (raw `.message`) log in `logDataRetentionTerminalAuditFailure()` were found in the same file during the sweep — **both predate this PR** (confirmed via `git diff origin/main...origin/feature/f3-impl-005-platform-admin-audit-final` showing zero touched lines at those locations) and are explicitly **out of scope** for this R1 fix, which is scoped to "every raw error logging site introduced by PR #365" per the assigning task brief; left as a candidate for a future, separately-scoped logging-hygiene sweep.

**Tests:** two new negative tests added to `platformFileBackupAudit.test.ts` (one per catch-path shape — a "completed"-branch failure and a "failed"-branch failure), each poisoning `prisma.$transaction` to reject with a synthetic error whose message embeds unmistakable sentinels (`patient@example.test`, `+905551234567`, `SECRET_TEST_TOKEN_9f3ab1`, a fake Postgres connection string) plus a `name`/`code`, capturing `console.error` during the call, and asserting: the caller-facing response is unaffected (a failed best-effort audit write must never block or alter it); exactly one log line was emitted; the serialized log line contains none of the sentinels or the connection string; the logged second argument is exactly `{errorCode, errorName}` with the expected values. `platformFileBackupAudit.test.ts`: **7/7** (5 pre-existing + 2 new). `platformAdmin.test.ts`: **118/118** unchanged (not touched by this fix). `cd server && npm run typecheck`: exit `0`. All log-privacy suites re-run for regression (`routeErrorLogPrivacy`, `httpRequestLogPrivacy`, `whatsappRouteLogPrivacy`, `externalCalendarWebhookLogPrivacy`, `instagramAiConversationProcessorLogPrivacy`, `whatsappConversationAgentLogPrivacy`, `inboundRateLimiterLogPrivacy`, `adminScriptsLogPrivacy`, `aiPrivacyBoundary`): all 0 failures. Full `npm run test:runtime:postgres -- --summary-file=postgres-run-summary.json` (`server:test:disposable-db`, includes the fixed `test:platform-file-backup-audit`) exit `0`; full `npm run test:runtime:postgres-compat -- --summary-file=postgres-compat-run-summary.json` (`server:test:legacy-db-required`, includes `test:auth`→`platformAdmin.test.ts`) exit `0` — both zero failures.

**No schema/migration change. No tenant/auth response-contract change.** Audit coverage inventory is unaffected by this fix (still 37/37 `AUDITED_DURABLY`, 0 `UNAUDITED_PERSISTED_MUTATION`) — only the *content* of the best-effort failure-path log line changed, not which mutations are audited or how. `RISK_REGISTER.md` row 92 (R-019) is left at `CLOSURE_PROPOSED_AWAITING_EXTERNAL_CONFIRMATION`, not marked `CLOSED` by this task, per the same R-071/R-072 precedent §8 already cites.

**Task status:** `AGENT_COMPLETED`/`TESTS_PASSED`/`PR_UPDATED` (pushed to existing [PR #365](https://github.com/MustafaBasol/DisKlinikCRM/pull/365), commit `77cdd8243222cb12e21ae2c4a7722642b1df72bf`) — `PR_CI_PENDING` (GitHub Actions had produced no new workflow run repository-wide for over 20 minutes after this push at the time of reporting — not specific to this PR/commit, not independently confirmed green by this task) — `NOT_MERGED`/`NOT_DEPLOYED`/`NOT_PRODUCTION_VERIFIED`.

## 12. R2-LITE — bounded `safeMetadata.errorType` classification on the class-5 failure paths (2026-08-12)

R1 (§11) replaced the raw `auditErr` object in the audit-write-failure `console.error` catch sites, but left a separate, durable-persistence defect unaddressed: both class-5 (§3.2) failure-branch audit rows persisted `safeMetadata: { errorType: err?.name ?? 'Error' }` — i.e. the underlying operation's own `err.name`, not the audit-write error. `Error.name` is ordinary, writer-mutable string data (`err.name = anything`); nothing in JavaScript or this codebase constrains it, so any thrower on the `runFileBackup()`/`runFileBackupRestoreRehearsal()` call path could in principle set a `.name` value carrying PII or secrets, and it would have been written verbatim into the durable `PlatformAdminAuditEvent.safeMetadata` column — not just logged.

**Fix:** added `boundedErrorType(err): 'Error' | 'UnknownError'` to `server/src/utils/safeError.ts` (`err instanceof Error ? 'Error' : 'UnknownError'` — no string from the thrown value is read or persisted, only the fact of whether it was a real `Error` instance). Applied at both PR-introduced file-backup failure `safeMetadata` sites in `server/src/routes/platformAdmin.ts`: `file_backup.manual_run.failed` and `file_backup.restore_rehearsal.failed`. The two structurally-identical DB-backup routes (`backup.manual_run.failed` / `backup.restore_test.failed`, `POST /backups/run` / `POST /backups/restore-test`) predate this PR (F3-IMPL-001) and are out of scope, same precedent §11 already applied to the analogous logging fix.

**Tests:** extended `platformFileBackupAudit.test.ts` with 3 new tests proving the bounded contract against a poisoned `Error` (`err.name` set to a string embedding a fake patient name/email/phone/token): both class-5 failure paths' `safeMetadata` literal (reconstructed exactly as built in `platformAdmin.ts`) classifies to `'Error'` and contains none of the sentinel values; a non-`Error` thrown value classifies to `'UnknownError'` rather than passing its own `name` property through. `platformFileBackupAudit.test.ts`: **10/10** (7 pre-existing + 3 new). `platformAdmin.test.ts`: **118/118** unchanged (not touched by this fix). `platformBackupAudit.test.ts`: **3/3** unchanged (out-of-scope routes, re-run for regression only). `cd server && npm run typecheck`: exit `0`.

**No schema/migration change. No tenant/auth response-contract change.** Audit coverage inventory is unaffected (still 37/37 `AUDITED_DURABLY`, 0 `UNAUDITED_PERSISTED_MUTATION`) — only the *shape* of the class-5 failure-path `safeMetadata.errorType` value changed (a fixed two-value label instead of passthrough `err.name`). `RISK_REGISTER.md` row 92 (R-019) is left at `CLOSURE_PROPOSED_AWAITING_EXTERNAL_CONFIRMATION`, not marked `CLOSED` by this task, per the same R-071/R-072 precedent §8 already cites. R-018 is unrelated to this fix and remains open.

**Task status:** `AGENT_COMPLETED`/`TESTS_PASSED` — not yet pushed to PR #365 as of this note; see the commit(s) immediately following this doc update in the branch history. `NOT_MERGED`/`NOT_DEPLOYED`/`NOT_PRODUCTION_VERIFIED`.
