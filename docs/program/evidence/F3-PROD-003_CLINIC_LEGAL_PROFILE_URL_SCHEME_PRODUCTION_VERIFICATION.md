...
>
> **Further amended 2026-08-17 by F3-PROD-003-R2 (same branch, same PR #435, documentation-only).** R1 missed one **still-active** stale rollback instruction, which survived inside the F3-PROD-003 paragraph of [`../phases/F3_PRODUCTION_HARDENING.md`](../phases/F3_PRODUCTION_HARDENING.md) — *"Rollback (repository-supported, backend only): revert to `b370b018…` and re-run `scripts/noramedi-deploy.sh`"*. That sentence now carries the same corrected contract this document states in §11 (runbook §4.1, mandatory `--skip-pull`, no `origin/main` rewrite, no DB rollback, manual frontend restore). **Nothing in this document changed as a result** — §5.1 and §11 were already correct — and no finding, status, SHA, count, or lifecycle state changed anywhere.
