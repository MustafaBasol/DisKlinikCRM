...
**Stage 0 characterization coverage (from the merged F2-PREP-007-E consolidation, re-confirmed present on `main`):** 21 of 21 blocking tests implemented and passing (all 18 mandatory-before-refactor + all 3 mandatory-before-caller-migration); 11 non-blocking tests (`CT-01,04,09,15,18,20,22,24,25,29,31`) explicitly not yet implemented, carried forward, non-gating. `server/package.json`'s `test:imaging-characterization` script exists on `main` (confirmed by direct read, §on file) and is wired into the CI-owned `server:test:disposable-db` aggregate, itself run by `ci-layers.yml`'s Layer 3 job (part of the passing run cited above).
