...
**Commit:** `92f77f414cf64b465716c85913c6c65fead85e5e` (code), `560acdd74d0751456681706dfd238c29539cb845` (docs), `ea2d0c0` (docs, PR #344 green-CI record), `39364f117874668c2bde34fe1fbca03de7430245` (fix — `F2-STAGE3-IMPL-001-R1` mutation-outcome amendment, current head)
**PR:** [#344](https://github.com/MustafaBasol/DisKlinikCRM/pull/344), current head `39364f117874668c2bde34fe1fbca03de7430245` (supersedes original head `560acdd`), base `main`, `OPEN`/`MERGEABLE`, not merged. **Exact-head PR CI: `success`** (independently re-verified via `gh pr checks 344`/`gh run list` at head `39364f1`): `ci` run [`31310907422`](https://github.com/MustafaBasol/DisKlinikCRM/actions/runs/31310907422) `SUCCESS` (all `ci-layers` jobs, including Layer 3 disposable PostgreSQL — the job that actually re-runs CT-05/CT-23/CT-30 and `privacyImagingLifecyclePortMigration.test.ts` in CI, not just locally), `windows-bridge-pr` run [`31310907297`](https://github.com/MustafaBasol/DisKlinikCRM/actions/runs/31310907297) `SUCCESS`. (The `31307552547` run cited in earlier drafts of this document was against the pre-`R1` head `560acdd`, now stale.)
...
Exact current commit chain on this branch, oldest first: `92f77f4` (code — this task's migration) → `560acdd` (docs) → `ea2d0c0` (docs, PR #344 green-CI record) → `39364f1` (fix — `F2-STAGE3-IMPL-001-R1` mutation-outcome amendment, current head). `git revert` those four commits in reverse order (`39364f1`, `ea2d0c0`, `560acdd`, `92f77f4`) restores the pre-migration direct Prisma `ImagingImage`/`ImagingStudy` access in both `patientAnonymization.ts` and `orphanFileInspection.ts` exactly, since no schema/migration/route file is touched by any of the four. No schema rollback needed or possible (none was ever applied). To roll back only the `F2-STAGE3-IMPL-001-R1` mutation-outcome amendment and keep the underlying migration, `git revert 39364f1` alone is sufficient — see that amendment's own evidence doc §15 for detail.
...
- `PR_OPENED`: TRUE — [PR #344](https://github.com/MustafaBasol/DisKlinikCRM/pull/344), current head `39364f117874668c2bde34fe1fbca03de7430245` (supersedes original head `560acdd`)
- `PR_CI_PASSED`: TRUE — exact-head CI independently re-verified via `gh pr checks 344`/`gh run list` at head `39364f1`: `ci` run `31310907422` `SUCCESS`, `windows-bridge-pr` run `31310907297` `SUCCESS`
