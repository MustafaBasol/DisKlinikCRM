...
> **Further reconciliation (F1-003-R1B, 2026-07-29):** `origin/main` has since advanced past F1-003-B1's merge (PR #258, merge commit `bb4186793323485c71b91ad6939a0d8469f886cd`, mergedAt `2026-07-28T22:13:04Z`), which resolved this document's own §11 overdue-installments blocker — see §11a for the current, post-B1 state. F1-003-R1A's MinIO correction (`test:kvkk-lifecycle` PostgreSQL-required/MinIO-free, `test:file-backup-db-integration` the sole genuine MinIO target) is unaffected and retained. This document and its JSON companion remain part of open PR #256, not independently merged — cross-checked against, never a substitute for, F1-003-R1A's own canonical evidence files.
...
|---|---|---|---|---|---|---|---|
| `server:test:non-disposable` | 68 explicit members (P1-defined; `test:overdue-installments` deliberately last) | None (pure Node / in-process mock only) | **Canonical** — the authoritative "safe without disposable runtime" aggregate | N/A — no other aggregate duplicates these members | PR: yes; main: yes; nightly: yes; release: yes | **Historical (P1): exit 1** (sole known cause: `test:overdue-installments`, 7 passed/2 failed, pre-existing product/test drift — §11/§F1-003-B1). **Current (post-B1, 2026-07-29, F1-003-R1B): exit `0`, `test:overdue-installments` 9/9 — resolved, PR #258 `MERGED`, merge commit `bb4186793323485c71b91ad6939a0d8469f886cd`. See §11.** | No infra owner — no cleanup required (in-process only) |
...
| Future PR profile | `root: npm run build` + `server: npm run typecheck` + `server: npm run server:test:non-disposable` | None beyond Node/tsc | Canonical, per P1's `futureCiCommandMatrix.pr` | N/A | PR | **Historical (P1):** blocked on `test:overdue-installments` (§14) for a "required, must be green" gate. **Current (post-B1, F1-003-R1B):** no longer blocked — `server:test:non-disposable` exits `0`; a "required, must be green" gate is now usable without a known-failure carve-out. No CI workflow is implemented by this or any prior F1-003 subtask. | No infra owner |
...
> **RESOLVED 2026-07-28 by F1-003-B1 (PR #258, `MERGED`, merge commit `bb4186793323485c71b91ad6939a0d8469f886cd`, mergedAt `2026-07-28T22:13:04Z`) — recorded here 2026-07-29 by F1-003-R1B.** `server:test:non-disposable` now executes end-to-end at **exit `0`** (68/68 members), `test:overdue-installments` now **9/9 passed**. The narrative below is preserved unchanged as the original, historical P1/R1 evidence — it describes the state *before* F1-003-B1's merge, not the current state. See the resolution summary immediately after this blockquote for the current, post-B1 state.
...
**This was recorded as a hard prerequisite for P3 workflow activation** at the time this section was originally written. Neither F1-003-R1 nor F1-003-P1 fixed it, removed or skipped the test, or marked the non-disposable aggregate green — `server:test:non-disposable`'s status at that time was: **exit 1, by design/acceptance, not by defect** — the aggregate correctly propagated a real product/test drift rather than hiding it.

**Follow-up task defined (not executed by F1-003-R1; since executed and merged — see resolution below):**

**F1-003-B1 — Overdue Installments Baseline Drift Resolution.** Scope: resolve the deterministic 2-assertion drift in `server/src/tests/overdueInstallments.test.ts` against current production code's `'overdue'` status-literal behavior — either update the test's expectations to match current intentional product behavior, or fix a genuine product regression, whichever the drift's root-cause analysis determines. Must not weaken, skip, or `|| true` the test. Must not touch disposable-runtime provisioning. Exact acceptance criterion: `server:test:non-disposable` (68 members, unchanged membership) executes end-to-end with **exit 0**, all 68 members passing, no member removed or reclassified to make this true. **This reconciliation (F1-003-R1) does not execute F1-003-B1** — it was named here as the exact, precise prerequisite that had to be satisfied, separately, before any CI profile treats `server:test:non-disposable`'s exit code as a required, must-be-green PR/main gate.

### 11a. F1-003-B1 resolution (recorded 2026-07-29, F1-003-R1B)

F1-003-B1 ran in parallel with F1-003-R1/PR #256 (did not touch this branch or worktree, per its own authorization), root-caused the drift as **Case A — stale test, not a product defect** (commit `81323a88` deliberately extended the shared status contract to include the legacy `'overdue'` literal; the sibling `overdueReceivables.test.ts` was updated in that same commit, but `overdueInstallments.test.ts` was not), and fixed exactly `server/src/tests/overdueInstallments.test.ts` — 2 assertions + its docstring corrected to the current, already-deployed contract; no application/schema/migration/CI/Docker/Compose file changed; no assertion weakened, skipped, or removed (still exactly 9 assertions). Result: `test:overdue-installments` 9/9 (was 7/9); `server:test:non-disposable` exit `0` (was exit `1`), verified across two runs with byte-identical output. F1-003-B1 was subsequently externally reviewed and merged as **PR #258** (merge commit `bb4186793323485c71b91ad6939a0d8469f886cd`, mergedAt `2026-07-28T22:13:04Z`), confirmed an ancestor of `origin/main` by this reconciliation (F1-003-R1B) via `git merge-base --is-ancestor`. **`server:test:non-disposable`'s hard-prerequisite-for-P3 status is now satisfied — it is removed from P3's blocker list (§22 item 4 below removed accordingly).** F1-003-R1B did not re-execute this test; F1-003-B1's own merged evidence ([evidence/F1-003-B1_OVERDUE_INSTALLMENTS_BASELINE_DRIFT_RESOLUTION.md](F1-003-B1_OVERDUE_INSTALLMENTS_BASELINE_DRIFT_RESOLUTION.md)) is cited as authoritative, not re-run. Until any future PR/main CI wiring of `server:test:non-disposable` exists (none does today — no `.github/workflows/**` file has been created or enabled by any F1-003 subtask), this remains a documentation-level resolution, not a live CI gate.
...
4. ~~`F1-003-B1` (overdue-installments drift) — named, not executed, not scheduled by this reconciliation.~~ **RESOLVED 2026-07-28 (F1-003-B1, PR #258, `MERGED`) — removed from this list 2026-07-29 by F1-003-R1B. See §11a.**
...
## 23. Explicit non-claims (this reconciliation, F1-003-R1; updated 2026-07-29 by F1-003-R1B)

This task does **not** claim: disposable-Postgres or MinIO provisioning is implemented, running, or CI-ready; any test was executed by this reconciliation task (P1's own historical execution record and F1-003-B1's own merged execution record are cited, not re-run — see §11/§11a); a CI workflow was implemented, created, or enabled; a deployment occurred; production was accessed or verified; R-070 is resolved, mitigated, or closed (it remains `OPEN`); R-046 or R-071's status was changed by this task (both are read, not altered — R-046 `OPEN`, R-071 `CLOSURE_PROPOSED_AWAITING_EXTERNAL_CONFIRMATION`, neither `CLOSED`/`MITIGATED`); F1's exit gate is satisfied; F1 or F1-003 is complete; G1/G2 approval status has changed; the KVKK baseline is stable; **PR #256 is merged (still open as of F1-003-R1B, 2026-07-29)**; **this document (the F1-003-P2A design) is itself independently merged — it remains part of open PR #256, cross-checked against the F1-003-R1A correction on that same branch, never a separately merged artifact; the only merged items in this lineage are F1-003-P1 (PR #257) and F1-003-B1 (PR #258)**; F1-003-P2 (disposable Postgres/MinIO provisioning) is implemented — it has not started; this task reached any status beyond `AGENT_COMPLETED` / `PR_OPENED_AWAITING_REVIEW`.
...
22. ~~P3 workflow activation remains blocked by the overdue-installments baseline drift (§11/F1-003-B1) unless F1-003-B1 is separately completed.~~ **RESOLVED 2026-07-28 (F1-003-B1, PR #258, `MERGED`) — recorded 2026-07-29 by F1-003-R1B. See §11a. P3 remains blocked on F1-003-P2's own unstarted disposable-runtime provisioning/collision-avoidance scope (criteria 1-14 above), not on the overdue-installments drift.**
