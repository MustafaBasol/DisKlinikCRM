...
## 26. R1 — external (Copilot) review response

**Task:** F2-GUARDRAIL-IMPL-001-R1. **Scope:** resolve exactly the four open Copilot inline review threads on PR #323 (base head `dae2bf409d9d66ae8424563417a6f865685f443c`). No baseline redesign, no false-positive validation pass, no Prisma detection, no new scanning roots, no runtime/migration/entitlement change, no CI-blocking enforcement — none of that was in scope for R1 and none was done.

**Finding 1 — config array shape validation (`scripts/architecture-guardrail/lib/configLoader.ts`, `loadScanRoots`).** Previously `fileExtensions` was checked only for `Array.isArray(...) && length > 0`; a malformed value such as `[1]` passed validation and would later throw a raw `TypeError` from `relPath.endsWith(ext)` in `sourceDiscovery.ts`, not the intended `ConfigError`. `excludePatterns`, when present but not an array (e.g. a bare string), was silently coerced to `[]` instead of rejected. Fix: every `fileExtensions` entry is now validated as a non-empty string; `excludePatterns`, when present, must itself be an array, and every entry must be a non-empty string; any violation throws `ConfigError` (never a bare `TypeError`) with a message naming only the config file path and field, never file content. Valid existing configs are unchanged. Covered by 7 new focused tests in `__tests__/configAndDiscovery.test.ts` ("fileExtensions and excludePatterns shape validation (review finding 1)" section) plus 6 new fixture files under `__tests__/fixtures/`.

**Finding 2 — `--repo-root` / default `--baseline` consistency (`scripts/architecture-guardrail/cli.ts`, `parseArgs`).** Previously the default `baselinePath` was computed once at the top of `parseArgs`, rooted at `defaultRepoRoot`, before the argument loop ran — so `--repo-root=<path>` alone (without an explicit `--baseline=`) left the baseline still pointed at the original default repo root, silently comparing scan roots from one tree against a baseline file from another. Fix: baseline defaulting is now deferred until after the full argument loop; a `baselineExplicit` flag (set by either `--baseline=` or `--no-baseline`) is tracked independently of `options.baselinePath` during parsing, and only when it is still `false` after the loop is the default baseline re-derived from the *final* `options.repoRoot`. This makes the outcome independent of argument order. `--repo-root <path>` (space-separated, no `=`) is not and was not previously a supported form — the parser only recognizes `--flag=value`; this is unchanged and out of R1's scope. Covered by 5 new focused tests in `__tests__/cli.test.ts` ("--repo-root / default --baseline consistency (review finding 2)" section): default invocation, `--repo-root` alone, `--repo-root` + explicit `--baseline` (override wins), reversed argument order (override still wins), and `--repo-root` + `--no-baseline` (disable still wins).

**Findings 3 and 4 — locale-independent determinism (`scripts/architecture-guardrail/lib/report.ts`).** `String.prototype.localeCompare()` is ICU/locale-collation-dependent and was used for finding ordering (`sortFindings`) and for `scope.scanRoots`/`scope.excludePatterns` ordering — both flagged directly by Copilot. A targeted inspection of `report.ts` (the only file these two threads named) for any other `localeCompare` use turned up one more: `sortByFilePathThenMessage`, which orders `report.errors[]` and `report.warnings[]` and was equally locale-dependent; it is fixed for the same reason, within the same file, under review finding 3/4's "any other output-affecting guardrail sorting found during targeted inspection" instruction. All four call sites now use a single new `compareCodeUnits(a, b)` helper (`a < b ? -1 : a > b ? 1 : 0`, plain UTF-16 code-unit comparison, no `Intl.Collator`). Finding IDs, semantic classification, and the report JSON schema are unchanged — only the *order* of already-produced values can differ from pre-fix output for non-ASCII inputs. `localeCompare` use outside `report.ts` (`sourceDiscovery.ts`, `baseline.ts`, `config/generateDomainMap.ts`) was inspected and left untouched: those files are outside the two threads' own scope, outside R1's allowed-file list, `computeFindingId()` hashes only the five semantic tuple fields (never scan/discovery order), and `report.ts`'s own now-fixed sorts make the final emitted order deterministic regardless of upstream discovery/baseline-load order — so no correctness or determinism gap remains unaddressed by confining the fix to `report.ts`. Covered by 6 new focused tests in `__tests__/report.test.ts` ("locale-independent (code-unit) sorting for non-ASCII values (review findings 3 & 4)" section), using the Turkish characters ç/ğ/ı/İ/ö/ş/ü named in the task (exact expected order: ç(U+00E7) < ö(U+00F6) < ü(U+00FC) < ğ(U+011F) < İ(U+0130) < ı(U+0131) < ş(U+015F)) across `callerPath`, `callerSymbol`, `scope.scanRoots`, `scope.excludePatterns`, and `errors[].filePath`, plus one test asserting two independent builds over the same (reversed-order) non-ASCII input serialize to byte-identical JSON.

**Files touched (R1):** `scripts/architecture-guardrail/lib/configLoader.ts`, `scripts/architecture-guardrail/cli.ts`, `scripts/architecture-guardrail/lib/report.ts`, `scripts/architecture-guardrail/__tests__/{cli,configAndDiscovery,report}.test.ts`, `scripts/architecture-guardrail/__tests__/fixturePaths.ts`, 6 new fixture JSON files under `scripts/architecture-guardrail/__tests__/fixtures/`, and this evidence file. No file under `server/src/`, `src/`, `server/prisma/`, `.github/workflows/`, `package-lock.json`, or `server/package-lock.json` was touched. No dependency, migration, or CI-workflow change.

**Validation (R1, all commands re-run against the fix commit):** `npm run guardrail:test` — 74 passed, 0 failed, exit 0 (was 56 before R1; +18 new focused tests, 0 removed). `npm run typecheck:guardrail` — clean, exit 0. `npm run guardrail:scan -- --deterministic --repo-sha=<R1 commit SHA>` run twice, `diff`/`cmp` byte-identical, exit 0 both. Generated report JSON parse/shape-validated (all 14 top-level fields present). Prohibited-secret-pattern scan and absolute-path scan against the generated report — both clean. `npm run test:runtime:unit` — 74 passed, 0 failed, exit 0 (unaffected-regression check, unchanged from R0). `npm run build` — succeeded, exit 0. `cd server && npm run typecheck` (`prisma generate` + `tsc --noEmit`) — clean, exit 0. `git diff --check` — exit 0 (only benign LF/CRLF autocrlf warnings, no reported whitespace errors). No conflict markers found in any changed or new file. `actionlint` — not required; no `.github/workflows/**` file was touched by R1. `git status --short` confirmed only the files listed above changed, plus this agent's own untracked local validation scratch files (`tmp_run1.json`, `tmp_run2.json`, `run1_stderr.txt`, `run2_stderr.txt`), which were not committed.

**Status (R1):** `AGENT_COMPLETED` / `TESTS_PASSED` — PR #323 already `PR_OPENED` (unchanged, existing PR). `NOT_MERGED` / `NOT_DEPLOYED` / `NOT_PRODUCTION_VERIFIED` (unchanged). New-head CI status is not claimed in this document — it must be independently re-checked once the CI run against the new head SHA completes; see the delivery report accompanying this task for the exact SHA and PR link.
