...
The four skip flags were chosen deliberately as the **minimal sufficient invocation**: they suppress deploy-script steps 1–4 (`git pull`, `npm ci`, `prisma migrate deploy`, `prisma generate`). The previously unexercised lifecycle contract in steps 5–8 was exercised. **During steps 5–6, the package start scripts themselves executed `npx prisma generate` for both processes** (`server/package.json:10-11`; §6) — suppressing step 4 does not suppress this. **The mutation surface was therefore PM2 lifecycle plus regeneration of Prisma Client artifacts from the unchanged schema/lockfile**, not PM2 process state alone; see §6 and §11.3 for the precise classification and what was and was not measured.
...
| V12 | Zero stale "steps 5–8 and nothing else / PM2-only blast radius" *current* claims survive (R2) | `grep -rn '"and nothing else"\|"PM2 process state only"' docs/program/evidence/F3-PROD-002_DEPLOY_SCRIPT_EXECUTION_VERIFICATION.md docs/program/NORAMEDI_MASTER_TRACKER.md` (restricted to this file and the tracker's *current* F3-PROD-002 top entry — historical dated text in other tasks' own entries, e.g. `NORAMEDI_MASTER_TRACKER.md`'s F3-IMPL-002-PROD-RECON "exact next task" note written before this invocation ran, is out of this correction's scope and left unedited) | **PASS** — `0` matches outside this validation section's own check descriptions. §3.1 and the tracker's top entry now state: deploy-script steps 1–4 skipped, steps 5–8 exercised, `npx prisma generate` additionally ran inside `npm run start`/`start:worker` at both process startups — mutation surface is PM2 lifecycle plus Prisma Client regeneration, not "PM2 state only" or "nothing else" |

**Pass/fail count: 12 checks executed, 12 passed, 0 failed.**
...
**R2 correction (2026-08-12), edited in place on this same PR/branch, same convention as R1** — R1 left one stale blast-radius statement standing in §3.1 ("exercise steps 5–8 … and nothing else. Blast radius is therefore PM2 process state only"), inconsistent with this file's own §6 and its already-corrected §11.3. §3.1 now states precisely: deploy-script steps 1–4 skipped, steps 5–8 exercised, and `npx prisma generate` additionally ran inside both package start scripts during steps 5–6 — so the mutation surface is PM2 lifecycle plus regeneration of Prisma Client artifacts from the unchanged schema/lockfile, not PM2 state alone. The equivalent "exercising exactly steps 5–8 and nothing else" wording in `NORAMEDI_MASTER_TRACKER.md`'s *current* F3-PROD-002 top entry was corrected the same way. Added V12 to check for recurrence of this specific phrasing.
