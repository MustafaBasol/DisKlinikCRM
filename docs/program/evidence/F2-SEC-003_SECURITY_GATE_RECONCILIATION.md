...
Both claims were independently re-derived from direct source-code and test-file reads on current `main` (`be72eb9b`), corroborated by already-passing CI-owned disposable-PostgreSQL test results recorded on the security PRs' own PR CI and post-merge main CI (§1 above) — not re-executed locally by this task, and not merely cited from prior evidence documents without this independent re-verification.
