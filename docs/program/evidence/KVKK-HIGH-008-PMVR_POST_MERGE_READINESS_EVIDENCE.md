...
|---|---|---|---|---|---|---|---|---|
| `20260719120821_kvkk_high007_consent_reconciliation` | 63rd of 64 | #175 | New table `CommunicationConsentConflictBucket`; 1 new index on existing `OperationalEvent` | None | Low (`CREATE TABLE`/`CREATE INDEX` only, no table rewrite) | No | No down-migration (Prisma migrate has none by design); migration itself does not alter/delete any pre-existing object or data. Physical schema rollback (dropping the new table/index) is data-loss-free **only while the table is empty** — once conflict-bucket rows exist, dropping it destroys that operational evidence. See §11 | Old app code runs unaffected (new table unused); new app code requires the table present |
| `20260719155318_kvkk_high008_legacy_consent_correction` | 64th of 64 (last) | #180 | New enum `PatientLegacyConsentField`; new table `PatientLegacyConsentCorrection` + 4 FKs | None | Low (`CREATE TYPE`/`CREATE TABLE` only) | No | No down-migration; migration itself does not alter/delete any pre-existing object or data. Physical schema rollback (dropping the new table/enum) is data-loss-free **only while the table is empty** — once correction records exist, dropping it destroys newly created KVKK correction/audit evidence, not merely "the new table." See §11 | Old app code runs unaffected; new app code requires the table present |
...
6. **Rollback mechanism?** No pre-existing object or data is ever mutated by either migration, so a schema rollback (dropping the new objects) is data-loss-free **only for as long as those new tables remain empty**. Once either workflow has created rows (a conflict bucket, a legacy consent correction), those rows are newly created operational/KVKK evidence — dropping the tables at that point is destructive to that evidence, not to anything pre-existing. The default production cutback is therefore **disable the feature → redeploy the prior compatible application commit → retain the additive tables → forward-fix if needed**; physically dropping the new tables is not the normal rollback path once the feature has been active, and requires an explicit data-preservation/export decision first. See §11.
...
3. Advanced worktree to `origin/main`, reinstalled deps, ran `prisma migrate deploy` **twice**: first run applied PR #180's migration; second run reported `No pending migrations to apply`. Re-running `prisma migrate deploy` was a safe no-op because Prisma recorded the migration as already applied in `_prisma_migrations`. This validates Prisma migration-engine replay behavior; it does not prove that directly re-executing the raw `CREATE TABLE`/`CREATE TYPE` SQL is idempotent — that statement-level idempotence was not separately tested.
...
| Migration itself alters/deletes pre-existing objects or data | No — purely additive | No — purely additive |
| Physical schema rollback (drop the new objects) data-loss-free | **Only while the new table is empty.** Once conflict-bucket rows exist, dropping the table destroys that newly created operational data — it is not "no data loss," it is "no loss of anything pre-existing" | **Only while the new table is empty.** Once correction rows exist, dropping the table destroys newly created KVKK correction/audit evidence, not merely an unused table |
| Rollback data loss if dropped before any row is created | None | None |
| Rollback data loss if dropped after the feature has created rows | Yes — all conflict-bucket rows created since deployment | Yes — all legacy consent correction rows (and the audit trail depending on them) created since deployment |
| Application rollback (redeploy prior commit, keep the additive table) | Backward compatible — old code ignores the new table, no data loss regardless of row count | Backward compatible — old code ignores the new table, no data loss regardless of row count |
...
4. **Migration rollback**: **not the default cutback step.** Only considered if the schema itself is implicated (not expected, since the migration is additive-only), and only after an explicit data-preservation/export decision — physically dropping `PatientLegacyConsentCorrection` (and `PatientLegacyConsentField` enum) is data-loss-free only if no correction rows have been created yet; once rows exist, dropping the table destroys that KVKK correction/audit evidence. Verify no application code path still references it first. The default cutback for a post-activation issue is (1) disable the feature, (2) redeploy the prior compatible application commit, (3) retain the additive tables as-is, (4) forward-fix — not dropping tables.
...
## 13. Reconciliation addendum (2026-07-20, KVKK-HIGH-008-PMVR-R1, PR #184 production evidence)

While this task was open, a separate, parallel documentation branch merged into `main` as PR #184 (commit `cf12b05`), carrying operator-supplied production evidence that PR #175's and PR #180's migrations are applied in production, the workflow is deployed and reachable, and `COMMUNICATION_CONSENT_*` flags remain disabled. `origin/main` was merged into this branch (`git merge --no-ff origin/main`) to reconcile the two passes; see the resulting conflict resolution in `RISK_REGISTER.md` (R-046, R-061, R-062) and `KVKK_HIGH008_FREEZE_BOUNDARY.md` ("Post-merge update 3").

**This task's own findings above are unchanged by that merge** — they were, and remain, repository- and disposable-environment-only. **Evidence-quality note:** PR #184's production narrative is specific and self-consistent (migration counts, PM2 restart/health figures, flag states) but is not accompanied by a raw command transcript, hostname, or committed evidence artifact (e.g. a `_prisma_migrations` query result), so this reconciliation records it as operator-reported/Partial evidence, not as an independently-verified fact equivalent to this document's own §5-§7 disposable-database evidence. If accurate, it would mean §9's residual risk ("the moment this workflow is deployed... it becomes simultaneously available... with no ability to pilot or pause it") is now realized rather than prospective — which increases, not decreases, the urgency of the still-outstanding R-061 flag decision this document's §9 recommends. It does not change this document's Non-authorization statement (§12): this task still did not observe or touch production, and still does not declare the KVKK baseline stable.
