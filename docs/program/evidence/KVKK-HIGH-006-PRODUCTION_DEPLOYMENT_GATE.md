...
This document defines the exact, safe execution sequence for the **next** production task on KVKK-HIGH-006: a controlled deployment of PR #194–#203 to production, followed by a production-safe smoke verification. **It does not execute any of that sequence.** No production command in §3, §4, or §5 below has been run by this task or any prior task. R-071 and R-061 both remain `OPEN` at the end of this document — neither is proposed for closure here; closure criteria for each are stated in §7 and require future evidence this task does not supply.

## 1. Verified authoritative state at task start

`origin/main` was freshly fetched and confirmed at `6b51f8d4520acc2e2cf56001e8a85d1737f16e1f` (PR #203's own merge commit — matches the task's expected SHA exactly). All ten named PR merge commits were independently confirmed both to exist as objects in the local mirror and to be ancestors of that `origin/main` tip via `git merge-base --is-ancestor <sha> origin/main` (exit `0` for all ten), and independently cross-checked against `gh pr view <n> --json state,mergeCommit` (all `MERGED`, `mergeCommit.oid` matching the task's supplied SHA in every case):
...
| #203 | `6b51f8d4520acc2e2cf56001e8a85d1737f16e1f` | Disposable-PostgreSQL DB-backed verification, current `origin/main` tip |
...
- **Backend test code (13 new files):** `server/src/tests/{appointmentRequestRecordScope,dentalChartClinicScope,kvkkHigh006Batch2ClinicScope,kvkkHigh006Batch3ClinicScope,messagesRecordScope,planLimitsTargetClinicFix,reportsClinicScope}.test.ts` and 6 files under `server/src/tests/dbVerification/`.
...
**Discrepancy note:** [KVKK-HIGH-006-DISPOSABLE_POSTGRES_VERIFICATION.md](KVKK-HIGH-006-DISPOSABLE_POSTGRES_VERIFICATION.md) (PR #203) states "all 66 existing migrations applied cleanly." This document's own independent recount of the same commit's `server/prisma/migrations/` directory finds **65**, not 66. This is flagged here as an unresolved **±1 documentation discrepancy in a prior evidence document**, not a functional or security defect — the substantive finding (all migrations in the directory applied cleanly, in order, no failure) is unaffected either way, and this document does not attempt to silently correct or restate PR #203's own evidence file. Whoever runs §5's `npx prisma migrate status` in production should treat **65** as this document's own independently-verified expected count and flag any other number as worth investigating before proceeding, rather than assuming either prior number is authoritative.
...
echo "Expected target SHA: 6b51f8d4520acc2e2cf56001e8a85d1737f16e1f"
...
- **Target SHA:** `6b51f8d4520acc2e2cf56001e8a85d1737f16e1f` (or a later, separately-reviewed `origin/main` tip if one exists by execution time — see §3.2's abort condition if it silently differs).
...
## 10. Stale test-assertion note

`server/src/tests/messageTemplateWabaBinding.test.ts` contains one known stale literal-string assertion, independently re-confirmed by this document: the test at lines 194–197 asserts the source contains the exact literal `const { metaTemplateConnectionId, metaWabaIdSnapshot, ...safeTemplate } = template;`, but the actual source at `server/src/routes/messages.ts:776` now reads `const { metaTemplateConnectionId, metaWabaIdSnapshot, clinicId: _clinicId, ...safeTemplate } = template;` — Batch 3 (PR #196) inserted a `clinicId: _clinicId` extraction into the same destructure as part of KVKK-HIGH-006's record-owned-scope remediation, which broke this test's brittle exact-substring match. The underlying security property this test is meant to guard — `metaTemplateConnectionId`/`metaWabaIdSnapshot` never present in the API response — **still holds**, confirmed by direct read of the surrounding handler; this is a test-assertion staleness, not a functional or security regression. This document independently re-checked the sibling assertion at lines 200–203 (`message-templates list route strips...`, matching `messages.ts:131`) and confirms it is **not** stale — `messages.ts:131` has no `clinicId` extraction inserted into its own destructure, so that literal match still holds exactly. Only the one assertion at lines 194–197 is stale.

This staleness does not block the deployment gate defined above. A separate, parallel task may fix the assertion; this deployment gate does not depend on that fix unless a future change to repository CI makes the full test suite's exit code a hard gate for deployment (it is not, as of this document — `npx prisma migrate deploy`/`pm2 reload` in §5 do not depend on this test file's own exit status).
