...
## 15. Addendum (F2-GUARDRAIL-VAL-003-R1, same file, same PR #330, `AGENT_COMPLETED` / `TESTS_PASSED`, not merged) — review corrections and full re-derivation

An automated PR review flagged four defects in the four reproducible scripts (§9), all in `scripts/architecture-guardrail-validation/`:

1. **`verifyVal003Candidates.mjs` — loose basename matching.** The import-verification regex matched only the target's basename as a substring of the specifier text (e.g. `services/storage.ts` would false-positive-match an import from `../services/fileStorage.js`, since `"storage"` is a substring of `"fileStorage.js"`). Fixed by resolving each import specifier to its full normalized, extensionless, repo-root-relative module path (`normalizeModulePath`/`resolveRelativeSpecifier`) and requiring exact equality against the target's own normalized path — never a substring or basename check. Extensionless relative imports and same-basename modules in different directories are now handled correctly. 12 focused self-check tests added (`__tests__/verifyVal003Matcher.test.js`), including a regression fixture shaped after the exact `storage.ts`/`fileStorage.ts` false-positive.
2. **`generateVal003BaselineEntries.mjs` — docblock opener not stripped.** The line-stripping regex handled the `*` body marker and `*/` closer but not the `/**` opener, so the literal `"/**"` token leaked into 24 of the 88 entries' `acceptedEvidence` quoted-header text (e.g. `"/** metaInboundDelivery.ts — ..."`). Fixed with an explicit `stripDocCommentMarkers` helper (opener/body/closer/same-line-combined, all as plain `startsWith`/`endsWith` checks, not a fragile alternation regex) and extracted into a pure, exported `extractHeaderSnippet`. 8 regression tests added (`__tests__/generateVal003Header.test.js`), including one asserting the raw `"/**"` token never appears in extracted output.
3. **`verifyVal003AfterScan.mjs` — `explicit88Ids` computed but unused.** Now used as a genuine independent cross-check: `explicit88Ids` (derived from `accepted_baseline_entries.json`'s `sourceEdgeIds`) is asserted to exactly match `explicitFindingIds` (derived independently from `candidate_classification.json`'s `findingId`), and the script now hard-fails (throws) rather than merely logging if any of the required invariants do not hold: unattributed flips ≠ 0, `organizationDashboard` involvement ≠ 0, or the two independent explicit-finding sources disagree.
4. **`buildVal003Candidates.mjs` — repeated baseline filter.** `baseline.edges.filter((e) => e.proposedEnforcementKey)` was recomputed inside `analyze()` on every one of the 88 still-`NEW` findings. Precomputed once as `edgesWithKeys` outside the function.

**Full re-derivation from scratch, not merely re-run in place:** the baseline file was restored to its pre-VAL-003 71-edge state (`git show 46acae8...:docs/program/evidence/F2-GUARDRAIL-PREP-010-A_cross_domain_access_inventory.json`, the exact parent commit of this task's own commit) and the entire pipeline re-executed end-to-end with the corrected scripts: before-scan (×2, byte-identical, byte-identical to the original before-scan) → `buildVal003Candidates.mjs` (88 candidates, identical reason breakdown: 87 `NO_RELATED_BASELINE_ENTRY_AT_ALL` + 1 `CALLERPATH_MATCHES_BUT_OWNER_TARGET_ACCESS_DIFFERS`, identical to original) → `verifyVal003Candidates.mjs` with the corrected full-path matcher (**88/88 still confirmed present, 0 stale** — the corrected matcher changes nothing for this specific dataset because none of the 48 distinct real target files share a basename, but the underlying substring-match defect is real and now closed generically) → `generateVal003BaselineEntries.mjs` (88 new entries, same ID range `CDA-073..CDA-160`) → after-scan (×3, byte-identical to each other and to the original after-scan: 846 `NEW`/193 `EXISTING`) → `verifyVal003AfterScan.mjs` with the corrected attribution cross-check.

**Result — all 88 entries independently re-verified as still valid; 0 removed, 0 reclassified:** every one of the 88 `CDA-073..CDA-160` entries survived re-derivation with an unchanged `id`, `classification` (`ACCEPTED_PUBLIC_CONTRACT`), `risk` (`Low`), and `proposedEnforcementKey`. The **only** content change from the R1 fix is cosmetic: 24 entries' `acceptedEvidence` text had its quoted target-module header cleaned of the leaked `"/**"` token (docblock-opener fix, finding 2 above) — field-by-field diffed against the pre-R1 committed file to confirm `acceptedEvidence` is the *only* field that changed on any of the 88 entries. Exact changed IDs: `CDA-074, CDA-080, CDA-081, CDA-083, CDA-084, CDA-090, CDA-093, CDA-106, CDA-109, CDA-114, CDA-118, CDA-121, CDA-124, CDA-125, CDA-126, CDA-141, CDA-146, CDA-148, CDA-149, CDA-154, CDA-155, CDA-156, CDA-157, CDA-160`.

**Independent proof (corrected script, hard-asserted, not merely printed):** `explicitFindingsCount: 88`, `sweptInSiblingsCount: 90`, `totalFlippedCount: 178`, `unattributedFlipsCount: 0`, `organizationDashboardInvolvedCount: 0`, `attributionSourcesAgree: true` — identical to the pre-R1 figures (§6), now independently cross-checked from two separately-sourced evidence artifacts rather than asserted from one.

**Validation (this R1 pass):** `npm run typecheck:guardrail` clean (exit 0); `npm run guardrail:test` 74/74 passed (exit 0); `npm run test:runtime:unit` 74/74 passed (exit 0); `npm run guardrail:scan` (bare) exit 0; `git diff --check` clean (exit 0, benign CRLF notices only); all 30 tests in `scripts/architecture-guardrail-validation/__tests__/` (22 pre-existing + 8 new R1 regression tests for finding 2 + 12 new R1 self-check tests for finding 1 — some tests exercise multiple assertions) passed; all four corrected scripts run directly, each exit 0.

No scanner/matcher/application/schema/migration/tenant/auth code touched by this correction pass — same scope boundary as the original task. Not merged, not deployed, not production-verified.
