...
> **SECOND CORRECTION (added 2026-08-12 R2, same task, additive — nothing below is rewritten).** The **pre-deploy production SHA / rollback anchor `b21cae911a0aa3444ebcd6e714a92c4f0802608a` recorded below is not resolvable in this repository or on `origin`.** Re-verified six ways, all negative — `git cat-file -t`, `git rev-parse --verify`, `git branch -a --contains`, `git tag --contains`, a full object-database prefix scan (0 objects), and `git fetch origin <sha>`, which the **server itself** rejects with `upload-pack: not our ref`. So this is not a shallow-clone or local-GC artifact. **What that means, precisely:** the documented rollback anchor is **not inspectable, diffable, or checkout-able** from the repository — nobody can review what would be rolled back to. **It does *not* mean rollback was operationally impossible**; the production host may hold that revision in its own checkout, and the separately verified pre-deploy database dump (SHA-256 `de8bf398…`) is unaffected. **No replacement SHA is proposed or invented here.** *Remediation before this rollback procedure is relied upon:* record a **resolvable** commit, tag, or release reference — pushing or tagging the revision on `origin` if it exists only in the production checkout. Until then treat the anchor as **documented but unverifiable**. No new risk ID was minted; this is an evidence correction plus an open operational follow-up.
>
> **THIRD CORRECTION (added 2026-08-12 by `F3-PROD-002`, additive — nothing below or above is rewritten; this narrows the SECOND CORRECTION immediately above, which is preserved intact).** Fresh operator-supplied production evidence establishes that **`b21cae911a0aa3444ebcd6e714a92c4f0802608a` does exist on the production host**: the object is present in the production repository's object database, its type is `commit`, `git show` succeeds, and its subject is `fix(external-calendar): enforce practitioner eligibility on the mapping write path (F3-DIGIDENTIS-MAP-001-R1)`. **It is therefore no longer accurate to describe the object as globally nonexistent.** Everything the SECOND CORRECTION says about *this repository* and about `origin` remains true and re-verified at `origin/main` @ `0d1b4ae9597b07dc0509f992a2a05663c015388f`. **What the new evidence adds is decisive in a different way:** on production, `git merge-base --is-ancestor b21cae91… aa18b064267ff5846ae60f73889c3322030bd4a8` exits **`1`** — the commit is **not on current production's lineage**. **Corrected classification: `b21cae91…` is locally resolvable on production, but invalid as the documented rollback-lineage anchor for current production.** **The current valid production rollback anchor is `aa18b064267ff5846ae60f73889c3322030bd4a8`** — resolvable in this repository, an ancestor of `origin/main`, present on `origin`, and simultaneously the current production deployed SHA. *(Repository-side note, verified by F3-PROD-002: `origin/main` carries `152ebaf5e9ad9b2da822d286c9354a9755d63440` with the byte-identical commit subject, and that commit **is** an ancestor of `aa18b06…`. The pairing is consistent with `b21cae91…` being a pre-squash PR-branch commit whose squash-merged equivalent on `main` is `152ebaf` — recorded as an inference, not asserted as proven.)* Full analysis: [F3-PROD-002_DEPLOY_SCRIPT_EXECUTION_VERIFICATION.md](F3-PROD-002_DEPLOY_SCRIPT_EXECUTION_VERIFICATION.md) §11.
>
> Full analysis, repository-side corroboration, and re-assessed R-033/R-034/R-040 dispositions: [F3-IMPL-002-PROD-RECON_PRODUCTION_WORKER_CONTRACT_VERIFICATION.md](F3-IMPL-002-PROD-RECON_PRODUCTION_WORKER_CONTRACT_VERIFICATION.md) §4–§6 — note **R-034 is `CLOSED` as of R2** (§6.2) and the rollback-anchor finding is §5.2. **R-033 and R-040 are both now `CLOSED` as of 2026-08-12 by `F3-PROD-002`**, each on its own row's stated criteria — see [F3-PROD-002_DEPLOY_SCRIPT_EXECUTION_VERIFICATION.md](F3-PROD-002_DEPLOY_SCRIPT_EXECUTION_VERIFICATION.md) §9.1/§9.2.
