...
**Second reconciliation (same task, `origin/main` advanced again mid-session):** after this task's own code and docs commits (`2b71750`, `d9a72c8`), a re-fetch found `origin/main` had moved further, to `83701882bd0f48a34ca8f8b02beb8a779003e638` (PR #278 merge commit) — three more PRs (#270, #279, #278) had merged since the first reconciliation, none authored by this task. `git merge --no-ff origin/main` a second time — again **zero conflicts**; brought in `docs/program/evidence/F2-PREP-002_*`, `F2-PREP-004_*` (documentation-only), and PR #270's own `feat/fix(treatment-cases)` changes (new `treatmentProposalPdf.ts` service, PDF route, `pdfkit`/`@types/pdfkit` dependency, two new test scripts added to `server:test:non-disposable`). Second merge commit: `2aff72e744148a207723ff6e9c07619ddbc611cb`. `npm ci` re-run in both `server/` and root to pick up the new `pdfkit` dependency; the full regression suite (§6) was re-run in full against a freshly-provisioned disposable PostgreSQL container after this second merge and remained **29/29** for the retention suite, 42/42 for `test:data-retention`, exit `0` for `server:test:non-disposable` (now including the two new treatment-proposal-pdf tests) and for `test:runtime:postgres`. `git merge-base --is-ancestor origin/main HEAD` confirmed **true** again after this second merge. No schema/migration file was touched by either reconciliation merge or by this task's own commits.
