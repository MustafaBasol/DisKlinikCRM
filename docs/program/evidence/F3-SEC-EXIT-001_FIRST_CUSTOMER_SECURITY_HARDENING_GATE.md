...
10. **External error tracking** — confirmed absent in-repo (no Sentry or equivalent dependency); this is unchanged from F3-IMPL-001's own finding and remains an open F3 exit-gate item independent of this task (see §6). **[2026-08-22 addendum, `F3-C2-ERR-004-CLOSE`.]** This is now stale on its literal premise: a real GlitchTip `SENTRY_DSN` was activated in production and a synthetic event was verified twice (see `evidence/F3-C2-ERR-004_GLITCHTIP_PRODUCTION_VERIFICATION_EVIDENCE.md` §17), under an explicit, dated program-owner `TECHNICAL_ACTIVATION_AUTHORIZED = YES` sequencing/risk-acceptance override — **not** `LEGAL_EXTERNAL_APPROVED`, and **not** a resolution of the R5 packet's DECISION-1 through DECISION-5. This item is reclassified `TECHNICALLY_SATISFIED_PENDING_LEGAL_RATIFICATION`, not `PASS` — the overall F3 exit gate remains `NOT SATISFIED` regardless, because other, independently-sufficient reasons recorded elsewhere in §11/the master tracker (e.g. the MFA negative-test-coverage gap) are each alone sufficient and are unchanged by this task.
