...
**28 passed, 0 failed, 6 skipped locally** (Windows/Git-Bash — `flock` is genuinely absent on this development host; the six skipped assertion groups require a real lock) — **and, critically, 50 passed, 0 failed, 0 skipped on CI's `ubuntu-latest` runner**, the exact-head result on this branch's final commit (`df92fbd`), where every one of those six locally-skipped groups ran for real for the first time in this task's history. That CI run is what actually surfaced and let this task fix four genuine defects the original PR's own testing structurally could not reach (every runtime-lock assertion is skipped on a host without `flock`, and CI never got past the executable-bit failure before this review-fix round): (1) a naive single-directory PATH strip left `flock` still reachable via a usrmerge duplicate (`/bin` vs `/usr/bin`), so the restore-proof "flock absent" assertion silently ran the real workflow instead of failing closed; (2) `with_status_lock()`'s original `exec N>FILE` (a bash special builtin) permanently redirected the SCRIPT's own stderr to `/dev/null` on its first successful call, then — even after subshell-scoping fixed that — a bare `(...)` subshell statement returning non-zero was found to abort the whole calling script under `set -e` before its own exit status could ever be captured; both were replaced with flock's own simpler `flock FILE COMMAND [ARGS...]` form, which needs no bash `exec` at all; (3) a bare `[[ cond ]] && action` logging line was itself "a failing command" under `set -e` whenever `cond` was false — i.e. exactly on the success path — fixed by using `if`; and (4), the actual root cause of "Successful backup run" never completing: three of the four status-merge `node -e` scripts destructured `process.argv` as `const [, , a, b, ...] = process.argv`, skipping two leading slots as if a script-path placeholder existed — `node -e` has none, so positional arguments start at `argv[1]`, and every argument was silently read one slot early, crashing with an uncaught `ENOENT` the instant a real `flock` finally let this path execute to completion. Coverage includes: `bash -n` syntax for all three scripts; `--help`/unknown-flag handling; `--tag` validation; every precondition (`restic` absent, `RESTIC_REPOSITORY`/`RESTIC_PASSWORD_FILE` unset/missing/unreadable/non-absolute, source directory missing, `flock` absent for backup AND restore-proof); dry-run isolation (no status file written, nothing invoked); lock contention for backup AND restore-proof (each against its own, independent lock file); the three operation locks' and the shared status-write lock's distinctness (a static assertion that backup/check/restore-proof default to three separate lock files while all three share the SAME status-write lock); a deterministic reproduction of the status-write concurrency race (§5's "shared status document concurrency" — a waiting writer forced to block on the status lock must merge onto the freshest content instead of clobbering it); successful and failing backup/check runs with status-file-shape assertions; ping-URL success/`/fail` behavior; the log-privacy canary (a path-shaped token injected into fake `restic`'s output on both a passing and a failing run, asserted absent from every byte the wrapper ever prints); the restore-proof success path and a deliberately corrupted restore (checksum mismatch correctly fails closed); and a structural assertion that `restore-proof.sh`'s code (not its prose comments) never references the primary-storage source-directory variable.
