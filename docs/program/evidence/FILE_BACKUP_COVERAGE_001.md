...
| Versioning | Not implemented (see §11 — provider-native versioning is a documented future option, not built) | Still not implemented/verified at the bucket level — **required operator configuration**, not app-enforced; see §14. This codebase makes no `GetBucketVersioning` API call and claims no knowledge of the configured bucket's actual versioning state |
| Retention/lifecycle policy for backup copies | N/A (nothing exists to retain) | Not implemented — every successful backup is kept indefinitely; no expiry logic exists yet (see §11); required operator lifecycle configuration documented in §14 |
...
| Encrypted transport to backup destination | N/A | HTTPS via `@aws-sdk/client-s3`/`@aws-sdk/lib-storage` in S3 mode; **as of this pass, `FILE_BACKUP_S3_ENDPOINT` (when set) is fail-closed rejected in production unless it is `https://`** (`validateFileBackupS3Config()`, `fileBackupDestination.ts`) — see §14. Local mode has no network transport |
| Encrypted storage at backup destination | N/A | As of this pass, `FILE_BACKUP_S3_SSE` (`AES256`/`aws:kms`) is **requested on every write** (`ServerSideEncryption`/`SSEKMSKeyId` params) and **fail-closed required** in production — S3-mode backup refuses to start in production with no SSE mode configured (`validateFileBackupS3Config()`). This is an app-layer request only; it does not call `GetBucketEncryption` to confirm the bucket's own default-encryption configuration — see §14 |
| Ransomware resistance (immutability/object-lock) | N/A | Not implemented — no object-lock/legal-hold applied to backup copies yet; documented as a **required/recommended operator configuration**, not app-enforced, in §14 |
...
**Deletion/anonymization interaction (explicitly unresolved by this task, disclosed rather than glossed over):** `patientAnonymization.ts` never touches physical file bytes (by existing, documented design — `docs/compliance/53`), so an anonymized attachment's *backup* copy is equally never touched; there is no leak introduced by backup that anonymization doesn't already accept for primary storage. **Correction (see `FILE_BACKUP_COVERAGE_001_R1_INDEPENDENT_REVIEW.md`, PR #247 review):** this section originally named only one live physical-delete path. There are in fact **two**: `DELETE /api/patients/:patientId/attachments/:id` (`server/src/routes/attachments.ts`, gated on `legalHold: false`) **and** `DELETE /api/lab-orders/:id/attachments/:attId` (`server/src/routes/labOrders.ts`, which has **no legal-hold gate at all**). Both delete the primary DB row and physical file — but this task's backup job does **not** delete the corresponding backup copy (or `FileBackupEntry` row) when either happens, confirmed live against a disposable Postgres instance. A deleted attachment's bytes therefore persist in the backup destination indefinitely, restorable, until a future task adds delete-propagation or a retention/lifecycle policy. **This is a real, disclosed gap, not a silent one** — it is the direct file-backup analogue of the DB-side gap this program's own reconciliation already names ("Backup purge policy for deleted/anonymized data... explicitly undecided in code comments," `KVKK_FINAL_RECONCILIATION_20260726.md` §8 item 5) and requires the same legal-review input before it can be closed correctly (a naive immediate-delete-on-backup would defeat backup's own ransomware/accidental-deletion protection purpose; a naive never-delete approach conflicts with a future KVKK retention-period decision). Recommended follow-up task: `FILE-BACKUP-RETENTION-POLICY-001`, blocked on the same legal retention-period decision already named in `KVKK_FINAL_RECONCILIATION_20260726.md` §8 item 5.
...
- That versioning, object-lock/ransomware-immutability, backup-destination lifecycle/cost controls, or application-layer (client-side) encryption of backup bytes are implemented — none of them are (§3's coverage matrix marks each "Not implemented" plainly). The PR #247 follow-up hardening pass added a fail-closed *requirement* that S3-mode production backup request provider-side server-side-encryption (SSE) on every write and use a TLS endpoint (§14) — that is a request/precondition enforced in application code, not a proof that the bucket's own configuration (default encryption, versioning, object lock) is actually correct; this codebase makes no API call to verify any of those bucket-level settings.
...
## 14. Production S3 bucket hardening requirements (operator-managed, not app-enforced)

Added in the PR #247 review-fixes pass (`fix(backup): close restore symlink bypass, fix cross-process backup lock`, plus this follow-up hardening commit). **Production S3-mode file backup remains disabled** (`FILE_BACKUP_ENABLED` still defaults to `false`; nothing in this section enables it) — this section only tightens what is *required* of an S3 configuration before it could safely be turned on, and documents what an operator must additionally configure at the provider/bucket level that application code cannot enforce.

**What the application now enforces in code (`fileBackupDestination.ts`'s `validateFileBackupS3Config()`, called from `getS3()` and from `fileBackupService.ts`'s `runFileBackup()` pre-flight, before any S3 client is constructed or `FileBackupRun` row created):**

- **TLS endpoint required in production.** `FILE_BACKUP_S3_ENDPOINT`, if set, must start with `https://` when `NODE_ENV=production`. An explicit `FILE_BACKUP_S3_ALLOW_INSECURE_ENDPOINT=true` is required to override this for a non-TLS-terminated, private-network-only endpoint. Leaving `FILE_BACKUP_S3_ENDPOINT` unset (real AWS S3) is always fine — the AWS SDK's default endpoints are always HTTPS.
- **Server-side encryption mode required in production.** `FILE_BACKUP_S3_SSE` (`AES256` or `aws:kms`, optionally paired with `FILE_BACKUP_S3_SSE_KMS_KEY_ID`) must be set before S3-mode backup will run in production — an unset SSE mode is a fail-closed startup/pre-flight error, not a silent fallback. When set, the resolved `ServerSideEncryption`/`SSEKMSKeyId` parameters are attached to every `PutObject` and multipart `Upload` call this module makes (both the actual file-backup writes and the JSON run manifest write).
- **Malformed SSE value always rejected**, in every environment (not just production) — an unrecognized `FILE_BACKUP_S3_SSE` value throws immediately rather than silently sending no encryption header.
- None of the above applies to `local`-mode destinations (`FILE_BACKUP_LOCAL_DIR`), which have no network transport or provider-side encryption story to validate.
- **Explicitly NOT implemented by this check:** no S3 API call is made to confirm the *bucket's own* configuration (default encryption, versioning, object lock, lifecycle rules, or the IAM policy attached to the credentials in use). The application requests encryption on its own writes and requires TLS for its own connection; it has no way to inspect or enforce provider-side bucket settings without additional, not-yet-implemented API calls (e.g. `GetBucketEncryption`, `GetBucketVersioning`, `GetObjectLockConfiguration`, `GetBucketLifecycleConfiguration`). **This document does not claim, and explicitly disclaims, that this application can prove any provider-side bucket setting below is actually enabled.** Where no such real API check exists, the status is kept explicitly as `IMPLEMENTED_NOT_PRODUCTION_VERIFIED` (or, where nothing at all is implemented, plainly "not implemented") — never upgraded to a stronger claim without an actual, working verification call added to the code.

**Bucket versioning — REQUIRED, operator-configured. Status: `IMPLEMENTED_NOT_PRODUCTION_VERIFIED`.**
Enable S3 bucket versioning on the backup destination bucket before production use. Versioning protects against a bug or compromised/misused credential overwriting or corrupting an existing backup object — with versioning on, the prior version remains recoverable even after an unwanted overwrite. This codebase does not call `GetBucketVersioning` (or any other API) to confirm versioning is actually enabled on the configured bucket, and makes no claim that it is. If a future task adds a real `GetBucketVersioning` check (e.g. a periodic self-check job, or an admin-route pre-flight that surfaces the result on `GET /file-backups/status`), this status may be reconsidered — until then it must not be upgraded past `IMPLEMENTED_NOT_PRODUCTION_VERIFIED`.

**Lifecycle / retention configuration — REQUIRED, operator-configured. Status: not implemented, not verified.**
Configure an S3 lifecycle rule appropriate to the clinic's retention obligations once the legal retention-period decision named in §8/§11 (blocked on `KVKK_FINAL_RECONCILIATION_20260726.md` §8 item 5) is actually made. **Do not configure an automatic-expiry lifecycle rule before that decision lands** — this application has no delete-propagation logic (§8's documented gap), so an automatic expiry rule set today could delete the only remaining copy of a file whose primary-storage copy has already been deleted, with no application-level awareness that this happened. Interim recommendation: no expiry lifecycle action; an optional storage-class transition rule (e.g. to colder/cheaper storage after N days) is safe for cost control since it never deletes data. Not implemented or verified by application code — this is provider configuration this codebase does not touch.

**Object lock / immutability — RECOMMENDED, operator-configured. Status: not implemented.**
For genuine ransomware/accidental-deletion resistance beyond versioning, enable S3 Object Lock (Governance or Compliance mode, per the clinic's risk tolerance) with a retention period on the backup bucket. On AWS S3 this must be enabled at bucket-creation time — it cannot be retrofitted onto an existing bucket — so this should be decided before the bucket is first created if pursued. Not implemented or verified by application code; tracked as a future hardening item, not claimed as done in any form.

**Least-privilege IAM policy — REQUIRED, operator-configured. Status: not implemented, not verified.**
The credentials supplied via `FILE_BACKUP_S3_ACCESS_KEY_ID`/`FILE_BACKUP_S3_SECRET_ACCESS_KEY` should be scoped to a dedicated IAM principal (never a shared/root/admin key) with a policy granting only what this codebase actually performs:
  - `s3:PutObject`, `s3:GetObject`, `s3:ListBucket` scoped to the specific backup bucket/prefix (`file-backups/*`) — required for backup writes, restore reads, and manifest read/write.
  - Explicitly **deny** `s3:DeleteObject`/`s3:DeleteObjectVersion` for this principal wherever operationally possible. This application never calls a delete API against the backup destination (§8's documented gap is that it never deletes anything there, not that it needs delete permission) — denying delete at the IAM layer makes that omission a defended property of the credential, not merely an accident of the current code.
  - No `s3:PutBucketPolicy`, `s3:PutBucketVersioning`, `s3:PutEncryptionConfiguration`, `s3:PutObjectLockConfiguration`, `s3:PutLifecycleConfiguration`, or any other bucket-administration permission — bucket-level configuration changes (the items above) should require separate, more-privileged, human-operated credentials, never the credentials embedded in the running application process.
Not implemented or verified by application code — this is IAM configuration external to this codebase, to be applied by whoever provisions the production bucket per §10's rollout plan.

**Monitoring / alerting conditions — RECOMMENDED, not implemented in this pass** (consistent with §9's existing disclosure that no alerting/paging integration exists yet):
  - No successful (`status: 'completed'`) `FileBackupRun` within the expected cron interval (`FILE_BACKUP_CRON`) plus a grace window — staleness signal.
  - Any `FileBackupRun` with `status: 'failed'`, or any completed run whose `filesFailed`/`filesMissing` count is non-zero — today this only surfaces via a manual `GET /api/platform/file-backups/status` check; there is no push/pull alert.
  - Provider-side signals outside this application's visibility, to be wired up operator-side against the bucket/IAM configuration above: elevated S3 request error rates (4xx/5xx) against the backup bucket, and — given the IAM deny-delete policy recommended above — any `DeleteObject`/`DeleteObjectVersion` call against the backup bucket via CloudTrail (or the provider's equivalent) at all, since any occurrence would indicate that deny policy was bypassed, misconfigured, or a different, more-privileged credential was used against this bucket.
  - Not implemented in this pass; documented as a requirement for whoever picks up an observability integration once this program's ADR-012 (currently `DEFERRED`, per §9) is resolved.
