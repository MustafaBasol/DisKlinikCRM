...
| Merge commit | `8906e66af5169220a4aed48fe4cfea8524976fb8` | `gh pr view 212 --json mergeCommit` |
| Ancestor check | `git merge-base --is-ancestor 8906e66af5169220a4aed48fe4cfea8524976fb8 origin/main` → exit `0` | Independently re-run by this task against `origin/main` @ `5f27ab15996d6f001d2da5f2f7be2ddd6ccfae0c` |
| Code drift since merge | **None.** `git log --oneline 8906e66..origin/main -- server/src/routes/appointmentRequests.ts server/src/services/appointmentRequestSafety.ts server/src/services/appointments/appointmentAvailabilityService.ts server/prisma/schema.prisma server/prisma/migrations` returns empty (24 commits exist between the two SHAs, none touch these paths) | Independently re-run by this task |
| **Verified deployment SHA** | `8906e66af5169220a4aed48fe4cfea8524976fb8` | See §1.1 below — inferred, not read directly from a live `git rev-parse HEAD` on the production host by this task |

### 1.1 How the deployment SHA was established (explicit method statement)

This task did **not** receive a raw `git rev-parse HEAD` reading captured on the production host. The deployment SHA above is established by two independent, converging lines of evidence, and is recorded with that basis stated explicitly rather than presented as a direct read:

1. **Prepared deployment target.** [`DATA-INTEGRITY-001_PRODUCTION_DEPLOYMENT_GATE.md`](DATA-INTEGRITY-001_PRODUCTION_DEPLOYMENT_GATE.md) — the only deployment procedure this program ever defined for this fix — fixes its target SHA at exactly `8906e66af5169220a4aed48fe4cfea8524976fb8` (§2, §4.1) and its own stop condition would have halted deployment on any SHA mismatch.
2. **Behavioral fingerprint match.** The production observations in §3–§8 below reproduce this commit's exact, verbatim response bodies — `{"error":"Appointment request is already converted"}` (HTTP 400) and `{"error":"Practitioner already has an appointment during this time","code":"APPOINTMENT_OVERLAP"}` (HTTP 409) — independently re-confirmed present verbatim in `server/src/routes/appointmentRequests.ts:224,281,406,416` at this task's baseline. No other commit in this repository's history introduces or alters these exact strings for this route.

Since zero commits between `8906e66` and current `origin/main` touch the relevant files (§1), any production deployment at or after `8906e66` that has not since been rolled back necessarily runs this exact code. This is recorded as the best available basis, not as an independently-witnessed `HEAD` read — a materially weaker evidentiary claim than entries elsewhere in this program that quote a live operator-run `git rev-parse HEAD`, and is stated as such rather than overstated.
...
- Every created row carried the **`DI001_PROD_SMOKE_*`** marker.
...
- **No pre-existing patient, appointment, or appointment-request record was intentionally modified, read into evidence, or deleted.** Only rows created by this verification run were touched.
...
No pre-existing Gebze Diş Dünyası record was read into, or altered by, this cleanup.
...
- **KVKK/PII exposure:** zero real patient PII was used, queried, or exposed at any point — every fixture used synthetic identity data per §3. No pre-existing patient record was read or altered.
