...
| Status | `AGENT_COMPLETED` — see §6 F0-009 entry for full detail. PR to be opened against `main`. |
...
- **Allowed next status:** `AGENT_COMPLETED` → PR açıldıktan sonra `PR_OPEN`; sıradaki adım dış inceleme/merge kararı; `MERGED`/`DEPLOYED`/`PRODUCTION_VERIFIED` yalnızca dış kanıtla kaydedilebilir.
