...
Son güncelleme: 2026-07-19 (F0-007 — KVKK aktif çalışma taban çizgisi ve mimari dondurma sınırı tamamlandı; F0-006'nın kendi `PR_OPEN` iç kaydı, [PR #173](https://github.com/MustafaBasol/DisKlinikCRM/pull/173)'ün `gh pr view 173` ile teyit edilmiş `MERGED` durumuyla düzeltildi — merge commit `91276dc7f610ef6923e3c1a7572f0ebba578a2f7`, `mergedAt: 2026-07-19T12:54:43Z`; ayrıca PR #167'nin bayat `OPEN` kaydı `MERGED` olarak düzeltildi)
...
|---|---|
| ID | F0-007 |
| Title | Active KVKK Work Baseline and Architecture Freeze Boundary |
| Status | `AGENT_COMPLETED` — pending PR. KVKK/privacy/security PR'larının tamamı GitHub üzerinden yeniden doğrulandı (repository-çapında sweep: 61/61 PR `MERGED`, 0 `OPEN`); KVKK-HIGH-007'nin taban özelliğinin ([PR #169](https://github.com/MustafaBasol/DisKlinikCRM/pull/169)) zaten merge edilmiş ve deploy edilmiş olduğu, aktif çalışmanın bunun devam/sertleştirme aşaması olduğu tespit edildi; PR #167'nin bayat `OPEN` kaydı düzeltildi; mimari dondurma sınır matrisi ve kanıt seti üretildi. Bkz. §6 F0-007. `MERGED`/`DEPLOYED`/`PRODUCTION_VERIFIED` ajan tarafından **atanamaz**. |
| Branch | `docs/f0-007-kvkk-baseline-freeze-boundary` |
| Worktree | `D:\Mustafa\Siteler\DisKlinikCRM-worktrees\f0-007-kvkk-baseline` (izole; ana KVKK-HIGH-007 çalışma ağacına dokunulmadı — yalnızca salt-okunur `git status`/`git branch`/`git rev-parse` çalıştırıldı, hiçbir dirty/untracked dosya içeriği okunmadı/diff alınmadı). Base: `origin/main` @ `91276dc7f610ef6923e3c1a7572f0ebba578a2f7` (F0-006'nın kendi merge commit'i, PR #173). |
| Scope | Yalnızca analiz/dokümantasyon: KVKK aktif çalışma taban çizgisi ve mimari dondurma sınırı (`docs/program/`) |
| Out of scope | Tüm uygulama, şema, migration, test, CI, deployment değişiklikleri; `docs/compliance/` değişiklikleri (bkz. R-042); birincil çalışma ağacına hiçbir yazma erişimi |
| Dependency | F0-002 (`MERGED`) |
| Reviewer | ChatGPT / kullanıcı |
| Evidence | [KVKK_ACTIVE_WORK_BASELINE.md](KVKK_ACTIVE_WORK_BASELINE.md), [KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md](KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md), [evidence/F0-007_KVKK_BASELINE_EVIDENCE.md](evidence/F0-007_KVKK_BASELINE_EVIDENCE.md), [evidence/F0-007_kvkk_work_inventory.json](evidence/F0-007_kvkk_work_inventory.json) |
...
- **Risks recorded:** R-041…R-052 ([RISK_REGISTER.md](RISK_REGISTER.md)) — 6 HIGH (migration/mimari-refactor çakışması, kanal-arası consent enforcement tutarsızlığı, yetkisiz backfill/reconciliation, retention/consent şema-geçiş çakışması, consent-contract bypass riski, rollback/tenant-etki kanıtsız migration) + 6 MEDIUM (stale compliance-doc drift, PR durum senkronizasyon boşluğu, CI kapsamı boşluğu, dokümantasyon bayatlama riski, undocumented config bayrağı riski, çakışma-çözme kavram örtüşmesi).
...
**External review and merge decision for the F0-007 PR** (to be opened, base `main` — see §6 F0-007 and §5).

F0-007 (Active KVKK Work Baseline and Architecture Freeze Boundary) tamamlandı → `AGENT_COMPLETED`; sıradaki adım PR açılması ve dış inceleme/merge kararı. `MERGED`/`DEPLOYED`/`PRODUCTION_VERIFIED` dış teyit gerektirir ve bu turda **atanmaz**. F0-007 yalnızca **analiz/dokümantasyon** görevidir; uygulama davranışını, şemayı, migration'ları, testleri, CI'ı, deployment'ı değiştirmedi; birincil çalışma ağacına hiçbir yazma erişimi kullanılmadı.

F0-002 ([PR #172](https://github.com/MustafaBasol/DisKlinikCRM/pull/172)), F0-003 ([PR #168](https://github.com/MustafaBasol/DisKlinikCRM/pull/168)), F0-004 ([PR #170](https://github.com/MustafaBasol/DisKlinikCRM/pull/170)), F0-005 ([PR #171](https://github.com/MustafaBasol/DisKlinikCRM/pull/171)) ve F0-006 ([PR #173](https://github.com/MustafaBasol/DisKlinikCRM/pull/173)) hepsi `main`'e merge edilmiş durumda.

F0-007 `main`'e merge edildikten sonraki **kesin sonraki görev: F0-008 — ADR Review and Enterprise Foundation Decision Set** (bağımlılığı F0-003 + F0-004, ikisi de `MERGED` — `READY`). Roadmap sırası atlanmamalıdır (görev talimatı: "Do not skip F0-008 without explicit documented authorization"). **F0-011 — Object Storage and Backup Migration Design** de artık bağımlılık-hazır (F0-002 + F0-006 ikisi de `MERGED`) ve **F0-012 — Controlled Pilot and General Launch Gate Definition** de bağımlılık-hazır (F0-006 + F0-007) — ancak ikisi de yalnızca **tasarım** görevleridir ve F0-011'in başlatılması görev talimatı gereği ayrı bir kullanıcı kararı gerektirir. F0-009/F0-010 implementasyonu (tasarımın ötesi) ve F1 faz girişi, KVKK-HIGH-007 devam çalışması dış teyitle stabilize olana kadar bloklu kalır — bkz. [KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md](KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md) §6/§7.
