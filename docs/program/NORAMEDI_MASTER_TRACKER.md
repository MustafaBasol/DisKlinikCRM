...
| Status | `AGENT_COMPLETED` — depo/kaynak-kod seviyesinde süreç/deployment/config/storage/backup izi ve F0-002 Stage B (+ aynı-günlü ikinci bir görev-sağlanan production kanıt anlık görüntüsüyle mutabakat) tamamlandı. Bkz. §6 F0-006. `MERGED`/`DEPLOYED`/`PRODUCTION_VERIFIED` ajan tarafından **atanamaz**. |
...
- **Allowed next status:** Dış inceleme sonrası `REVIEW_REQUIRED`/`CHANGES_REQUESTED`/PR akışı — bu turda PR açılmasıyla `PR_OPEN`; `MERGED`/`DEPLOYED`/`PRODUCTION_VERIFIED` yalnızca dış kanıtla kaydedilebilir.
...
**External review and merge decision for F0-006's pull request** (bu turda açılacak — bkz. §6 F0-006 ve §5).

F0-006 (Production Topology and Configuration Verification) tamamlandı → `AGENT_COMPLETED`, ardından PR açılmasıyla `PR_OPEN`. `MERGED`/`DEPLOYED`/`PRODUCTION_VERIFIED` dış teyit gerektirir ve bu turda **atanmaz**. F0-006 yalnızca **analiz/dokümantasyon** görevidir; uygulama davranışını, şemayı, migration'ları, testleri, CI'ı, deployment'ı, Nginx'i veya PM2 config'ini **değiştirmedi**; production'a hiçbir yazma erişimi kullanılmadı.
