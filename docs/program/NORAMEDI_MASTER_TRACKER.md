...
|---|---|
| ID | F0-009 |
| Title | RLS, Prisma, and PgBouncer Proof-of-Concept Design |
| Status | `PR_OPEN` — [PR #177](https://github.com/MustafaBasol/DisKlinikCRM/pull/177) açıldı, base `main`, dış inceleme bekliyor. See §6 F0-009 entry for full detail. |
| Branch | `docs/f0-009-tenant-rls-pgbouncer-poc-design` |
| Worktree | `D:\Mustafa\Siteler\DisKlinikCRM-worktrees\f0-009-tenant-rls-pgbouncer-poc` (izole; birincil çalışma ağacına — `D:\Mustafa\Siteler\DisKlinikCRM`, `feature/kvkk-high007-consent-reconciliation-ux` dalında ve temiz — yalnızca salt-okunur `git status --short`/`git branch --show-current`/`git rev-parse HEAD`/`git worktree list` çalıştırıldı, hiçbir dosya okunmadı/değiştirilmedi). Base: `origin/main` @ `9669b06aa19035d45ccdec85837b71c9e4e8512d` (F0-008'in kendi merge commit'i, PR #176). |
| Scope | Yalnızca analiz/dokümantasyon/tasarım: PoC tasarımı, model envanteri, deney matrisi, master tracker/faz dokümanı/ADR/risk kaydı güncellemesi (`docs/program/`, `docs/architecture/`). Hiçbir uygulama/şema/migration/deployment/CI dosyası dokunulmadı. |
| Out of scope | RLS uygulaması, Prisma guard uygulaması, PgBouncer kurulumu, herhangi bir migration, database rol değişikliği — hepsi F0-007 dondurma sınırı ve bu görevin kendi talimatı gereği açıkça bloklu. |
| Dependency | F0-004, F0-008 (ikisi de `MERGED`) |
| Reviewer | ChatGPT / kullanıcı |
| Evidence | [../architecture/tenant-rls-pgbouncer-poc-design.md](../architecture/tenant-rls-pgbouncer-poc-design.md), [../architecture/evidence/f0-009-tenant-model-inventory.json](../architecture/evidence/f0-009-tenant-model-inventory.json), [../architecture/f0-009-poc-test-matrix.md](../architecture/f0-009-poc-test-matrix.md) |
| Ajan için izinli sonraki durum | Dış inceleme sonrası `REVIEW_REQUIRED`/`CHANGES_REQUESTED`/PR akışı; `MERGED`/`DEPLOYED`/`PRODUCTION_VERIFIED` yalnızca dış kanıtla kaydedilebilir |
...
**External review and merge decision for the F0-009 PR** (to be opened against `main` as part of this task's own delivery — see §5, §6 F0-009). F0-009's PoC design document, 91-model tenant inventory, and 20-experiment test matrix are `AGENT_COMPLETED` and require external (ChatGPT/user) review before merge, per tracker §2.2/§2.3. This document does not authorize any implementation — see the design document's required non-authorization statement.

F0-002 ([PR #172](https://github.com/MustafaBasol/DisKlinikCRM/pull/172)), F0-003 ([PR #168](https://github.com/MustafaBasol/DisKlinikCRM/pull/168)), F0-004 ([PR #170](https://github.com/MustafaBasol/DisKlinikCRM/pull/170)), F0-005 ([PR #171](https://github.com/MustafaBasol/DisKlinikCRM/pull/171)), F0-006 ([PR #173](https://github.com/MustafaBasol/DisKlinikCRM/pull/173)), F0-007 ([PR #174](https://github.com/MustafaBasol/DisKlinikCRM/pull/174)), and F0-008 ([PR #176](https://github.com/MustafaBasol/DisKlinikCRM/pull/176), corrected from a stale `PR_OPEN` self-reference reading this task found — see §6 F0-008 and §7) are all `MERGED` into `main`.

**Reaffirmed by F0-009:** the KVKK-HIGH-007 continuation ([PR #175](https://github.com/MustafaBasol/DisKlinikCRM/pull/175)) remains `MERGED` (commit `1da9586995b625624b7385c14e70ba6a322def73`) but **not** production-verified. [KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md](KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md) §5 conditions 3-5 remain unsatisfied. F0-009's own design work is scoped entirely as `KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md` §6 "allowed now: design only" — no stage of its proposed 11-stage rollout was executed, and its own design document carries an explicit non-authorization statement.

**Exact next task after F0-009's external review:** either **F0-010 — Queue and Transactional Outbox Proof-of-Concept Design** (dependency-ready: F0-004 + F0-008, both `MERGED`; design-only, per [KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md](KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md) §6) or the first implementation-independent follow-up named in F0-009's own design document (§6: confirming/adding the upstream tenant-ownership check for `securityIncidentService.ts:138`'s raw SQL — a bug-fix/audit-correctness item, not a tenant-scoping architecture change, and explicitly not blocked by the freeze boundary's RLS/tenant-extension items). **F0-011 — Object Storage and Backup Migration Design** and **F0-012 — Controlled Pilot and General Launch Gate Definition** are also dependency-ready but each requires a separate user decision to start, per prior tasks' instructions. F0-013 (consolidated validation report) remains blocked until F0-009…F0-012 complete.
