...
**Baseline commit used:** `origin/main` @ `c49466e15daa66933bf5c2e36fe46343e08bfcdd` (`docs(program): F0-011-P1 — Active KVKK-HIGH-008 Work Baseline and Architecture Freeze Boundary (#181)`), verified via `git fetch origin --prune` + `git rev-parse origin/main` at this task's start.
...
| KVKK-HIGH-008 (legacy consent correction) | `MERGED` — [PR #180](https://github.com/MustafaBasol/DisKlinikCRM/pull/180), merge commit `e972bfef918471074137bb0f11705d43a1ca2ce5`. Migration `20260719155318_kvkk_high008_legacy_consent_correction` present in `main`. **Production migration application: unconfirmed. Deployment: unconfirmed. Feature activation: unconfirmed (workflow ships with no flag — R-061). Production verification: not performed.** | [KVKK_HIGH008_FREEZE_BOUNDARY.md](KVKK_HIGH008_FREEZE_BOUNDARY.md), [RISK_REGISTER.md](RISK_REGISTER.md) R-061/R-062 |
| KVKK-HIGH-007 continuation (consent reconciliation) | `MERGED` — [PR #175](https://github.com/MustafaBasol/DisKlinikCRM/pull/175), commit `1da9586995b625624b7385c14e70ba6a322def73`. Migration `20260719120821_kvkk_high007_consent_reconciliation` present in `main`. Enforcement flag `COMMUNICATION_CONSENT_ENFORCEMENT_ENABLED=false`, mode `audit`; reconciliation flag `COMMUNICATION_CONSENT_LEGACY_RECONCILIATION_ENABLED=false`. **Production migration application: unconfirmed. Production backfill: confirmed NOT executed** (`../compliance/KVKK_COMPLIANCE_AUDIT_AND_REMEDIATION.md` 2026-07-19 row). | [KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md](KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md) §5 |
...
- F0-012 (this document) itself reviewed and accepted — **not yet; this document is `AGENT_COMPLETED` at authoring time, per tracker §2.3 an agent cannot self-assign further.**
...
| External legal or supplier dependencies | KVKK compliance program's own self-assessment must move off "Conditional Controlled Pilot — not yet cleared" for the specific clinics chosen, OR the decision owner must explicitly accept operating below that self-assessment's own recommendation for a bounded, monitored pilot (this is itself a business/legal risk-acceptance decision, not a technical one — record it in §7, do not let a technical agent make it). VERBİS per-clinic determination and DPA/subprocessor agreement drafting do not need to be complete for every future clinic, but **must** be complete or explicitly time-boxed for the specific pilot clinics chosen. |
...
| Requirement | Detail |
|---|---|
| Tenant scoping | R-001's `PARTIALLY_VERIFIED` application-layer scoping (`clinicScope.ts`/`clinicAccess.ts`, ~40+ call sites) is the *only* tenant-isolation mechanism in production today — no RLS exists (ADR-005 `NEEDS_POC`). G1 must record this explicitly as an accepted architecture, not a gap to be silently closed; pilot clinics must be informed (at the program/business level, not this document's concern) that isolation is application-layer only. |
...
| Requirement | Detail |
|---|---|
| Migration chain verification | Confirm the full migration directory (60+ migrations at F0-002 baseline, now +2 with PR #175/#180) applies cleanly and linearly, no gaps/conflicts, via `npx prisma migrate status` against a disposable database — not yet performed for the two newest migrations. |
| Disposable PostgreSQL upgrade rehearsal | Run `npx prisma migrate deploy` against a disposable/local Postgres instance seeded from a production-shaped (not production-data) schema, starting from the migration state immediately before PR #175/#180's migrations, confirming both apply without error. Not yet performed — F0-005 documented this exact capability as blocked by disposable-Postgres unavailability at that time; re-attempt required before G1. |
| Clean-database migration rehearsal | `npx prisma migrate deploy` from zero (empty schema) through the full current chain must succeed — validates no migration silently depends on manually-applied out-of-band state. |
| Previous-release-to-current upgrade rehearsal | Starting from the confirmed production migration head as of F0-002 Stage B (`20260718164142_add_communication_preference_and_consent`), apply forward through PR #175 and PR #180's migrations in a disposable environment, confirming order and idempotency. |
...
| Rollback/cutback method | **None exists today** for `scripts/noramedi-deploy.sh` (fail-fast, not transactional, per [PRODUCTION_TOPOLOGY.md](PRODUCTION_TOPOLOGY.md)). For G1, at minimum a manual, documented rollback runbook (which migration(s) to reverse, `pm2 reload` to which prior commit, order of operations) must exist and be rehearsed once in a disposable environment before the pilot's first production migration. An automated rollback mechanism is **not** required for G1 (it is a reasonable G2/F3 requirement — see below) but its absence must be an explicit accepted risk, not silent. |
...
| Tenant-impact verification | Confirm both migrations are additive-only and do not alter existing tenant-scoped columns' semantics (per read-only review already performed by F0-011-P1 for KVKK-HIGH-008 — that review is evidence *of narrowness*, not evidence of *production rollback safety*, which remains a separate, unmet requirement per R-046). |
| Production migration status evidence | **Neither PR #175's nor PR #180's migration is confirmed applied to production as of this document.** This is the single largest concrete gate item — G1 cannot be `APPROVED` while both remain unconfirmed for a pilot that will exercise this exact schema. |
| Migration applied vs. migration present distinction | Explicitly: "migration file exists in `main`" ≠ "migration applied to the production database." Evidence for the latter requires a read-only production query (e.g., `SELECT * FROM "_prisma_migrations" WHERE migration_name IN (...)`, same pattern as F0-002 Stage B) — not yet performed for either migration. |
| **PR #175 / PR #180 explicit incorporation** | Both are `MERGED` repository facts. Neither has confirmed production migration application, deployment, feature activation (PR #180 has none by design — R-061), or production verification. R-046 (rollback/tenant-impact unconfirmed) and R-062 (migration-ordering risk) apply directly to any pilot-clinic database that receives these migrations. **G1 requires both R-046 and R-062 to move from `OPEN` to at least `MITIGATED` (with cited evidence) before the pilot's first clinic onboarding**, not merely "merged." |
...
| PITR | **Blocker for neither G1 nor G2 by itself** — remains an accepted, documented risk (R-031) as long as the backup-interval RPO exposure is explicitly accepted per above. Escalates to a **G2 mandatory item** (see below) once clinic count/data value crosses a threshold the decision owner sets. |
| Off-site backup | **Accepted temporary risk for G1** (small, monitored cohort; recoverable via same-host backup if same-host itself is not the failure mode) — **must become mandatory before G2** (see below), since general launch removes the "small, monitored, reversible" framing that justifies accepting this for G1. |
| File-tree (attachment/imaging) backup | **Accepted temporary risk for G1** if the pilot clinics' expected attachment/imaging volume is low and loss would be recoverable/re-obtainable; **must be explicitly evaluated per pilot clinic** — do not accept blanket. |
...
| KVKK-HIGH-008 legacy consent correction | Merged, **no flag exists** — activates immediately for OWNER/ORG_ADMIN/CLINIC_MANAGER upon deployment (R-061) | **A kill-switch decision is mandatory before this workflow may be part of any pilot deployment**: either (a) add a flag before deploying to pilot clinics, or (b) the decision owner explicitly accepts the no-flag, rollback-only-via-redeploy design for the pilot's risk profile, in writing (§7). Silence/deploying without this decision is not acceptable — this is precisely what R-061 exists to prevent. |
...
|---|---|---|
| KVKK legal review | Per-item, ongoing (compliance doc §2/§3) — several items still "waiting for legal review" | Must be complete or explicitly time-boxed **for the specific pilot clinics chosen**, not for the whole future clinic base. |
| VERBİS | Per-clinic decision tree, waiting for legal review | Same as above — resolve for pilot clinics specifically. |
| Subprocessor/DPA review | Platform↔clinic processor agreement not yet drafted | Must exist, signed, for pilot clinics before their data is processed under it — this is a legal/contractual artifact, not a technical one; this document cannot verify its existence, only require its citation. |
...
**The system must not be labeled legally compliant based solely on satisfying the technical items (§§A-G) above.** Technical readiness and legal/external readiness are tracked and signed off separately (§7 approval record has distinct signature lines for each).
...
3. R-061 with no explicit accept/reject decision recorded (KVKK-HIGH-008 no-kill-switch design).
...
### Accepted temporary risks (may proceed with explicit sign-off, not silently)
R-029 (local storage), R-030 (no offsite backup, conditioned on §E), R-031 (no PITR), R-033…R-040 (operational debt), R-054/R-055 (RLS design open question — no RLS exists regardless), the CI-coverage gap (manual test execution substitutes until F1 exists), the monitoring/alerting gap (manual monitoring substitutes for a small pilot).
...
- File-tree (attachment/imaging) backup: **mandatory** if any pilot or pre-launch clinic has accumulated attachment/imaging data of material volume/value — no longer clinic-by-clinic accepted risk.
...
Section H (External/legal):           [BLOCKED | CONDITIONAL | SATISFIED] — notes: [SIGNED OFF SEPARATELY — legal/business, not technical]

Accepted temporary risks (list, with individual sign-off per item):
1. ...
