...
| KVKK-HIGH-008 legacy consent correction | Merged, **no flag exists** (R-061). **Deployment confirmed** (2026-07-20, `origin/main` @ `cf12b05e9a65730e39b0aa667469e312fade5cc8`, PR #184: migration applied, table/enum present, 0 correction rows) — the workflow's routes are reachable/available to authorized roles (OWNER/ORG_ADMIN/CLINIC_MANAGER, manual authenticated API/UI action). **No automatic startup, cron, job, worker, or automatic UI invocation path was found by PMVR** (KVKK-HIGH-008-PMVR Phase B1, [PR #185](https://github.com/MustafaBasol/DisKlinikCRM/pull/185)) — whether the workflow has actually been *used* by an authorized user (as opposed to merely reachable) remains unverified; 0 correction rows as of the 2026-07-20 evidence is consistent with, but does not prove, zero invocations. Do not state or imply automatic activation on deployment. | **A kill-switch decision is mandatory before this workflow may be part of any pilot deployment**: either (a) add a flag before deploying to pilot clinics — PMVR's proposed `PlatformSetting`-backed runtime toggle (`privacy.legacyConsentCorrection.runtimeEnabled`, default disabled) is a candidate, not yet implemented or authorized — or (b) the decision owner explicitly accepts the no-flag, rollback-only-via-redeploy design for the pilot's risk profile, in writing (§7). This decision is now **overdue**, not merely pending — the workflow is live in production with no flag. Silence/deploying without this decision is not acceptable — this is precisely what R-061 exists to prevent. |
