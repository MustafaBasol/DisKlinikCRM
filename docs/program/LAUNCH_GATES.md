...
| KVKK-HIGH-008 (legacy consent correction) | `MERGED` — [PR #180](https://github.com/MustafaBasol/DisKlinikCRM/pull/180), merge commit `e972bfef918471074137bb0f11705d43a1ca2ce5`. Migration `20260719155318_kvkk_high008_legacy_consent_correction` present in `main`. **Status separation (see §0 non-collapse table):** code merged — confirmed. Migration present in repository — confirmed. Production migration applied — unverified by this document. Deployment — unverified by this document. **Deployment would make the endpoint/UI workflow reachable/available to authorized roles (OWNER/ORG_ADMIN/CLINIC_MANAGER); it is a manual, authenticated API/UI-invoked administrative workflow, not startup-, cron-, job-, or worker-driven — no automatic invocation path was found (KVKK-HIGH-008-PMVR Phase B1, [PR #185](https://github.com/MustafaBasol/DisKlinikCRM/pull/185), author-reported/`OPEN`, not independently re-verified by this document). Automatic execution is not established by this document and remains subject to PMVR/production evidence.** Feature activation — unverified. No explicit kill switch — confirmed repository finding (R-061); PMVR (PR #185) proposes an unimplemented `PlatformSetting`-backed runtime toggle (`privacy.legacyConsentCorrection.runtimeEnabled`, default disabled, no Prisma migration required) as one option — not adopted or authorized by this document. Production verification — not performed. | [KVKK_HIGH008_FREEZE_BOUNDARY.md](KVKK_HIGH008_FREEZE_BOUNDARY.md), [RISK_REGISTER.md](RISK_REGISTER.md) R-061/R-062, [PR #185](https://github.com/MustafaBasol/DisKlinikCRM/pull/185) |
...
| Production migration status evidence | **Neither PR #175's nor PR #180's migration is confirmed applied to production as of this document.** This is the single largest concrete gate item — G1 cannot be `APPROVED` while both remain unconfirmed for a pilot that will exercise this exact schema. Disposable-environment rehearsal evidence (PR #185) reduces schema-level ordering uncertainty but is **not** a substitute for production application evidence — do not close R-062 on rehearsal evidence alone. |
| Migration applied vs. migration present distinction | Explicitly: "migration file exists in `main`" ≠ "migration applied to the production database." Evidence for the latter requires a read-only production query (e.g., `SELECT * FROM "_prisma_migrations" WHERE migration_name IN (...)`, same pattern as F0-002 Stage B) — not yet performed for either migration. **R-062 remains `OPEN` until this specific production `_prisma_migrations` evidence is collected**, regardless of disposable-rehearsal results. |
| **PR #175 / PR #180 explicit incorporation** | Both are `MERGED` repository facts. Neither has confirmed production migration application, deployment, feature activation (PR #180 has none by design — R-061), or production verification. R-046 (rollback/tenant-impact unconfirmed) and R-062 (migration-ordering risk) apply directly to any pilot-clinic database that receives these migrations. **G1 requires both R-046 and R-062 to move from `OPEN` to at least `MITIGATED` (with cited evidence) before the pilot's first clinic onboarding**, not merely "merged," and not merely "rehearsed in a disposable environment" (PR #185's rehearsal evidence is a contribution toward this, not a substitute for it). |
...
| KVKK-HIGH-008 legacy consent correction | Merged, **no flag exists** (R-061). Deployment would make the workflow reachable/available to authorized roles (OWNER/ORG_ADMIN/CLINIC_MANAGER, manual authenticated API/UI action). **No automatic startup, cron, job, worker, or automatic UI invocation path was found by PMVR** (KVKK-HIGH-008-PMVR Phase B1, [PR #185](https://github.com/MustafaBasol/DisKlinikCRM/pull/185)) — feature activation remains unverified until production configuration and behavior are inspected; do not state or imply automatic activation on deployment. | **A kill-switch decision is mandatory before this workflow may be part of any pilot deployment**: either (a) add a flag before deploying to pilot clinics — PMVR's proposed `PlatformSetting`-backed runtime toggle (`privacy.legacyConsentCorrection.runtimeEnabled`, default disabled) is a candidate, not yet implemented or authorized — or (b) the decision owner explicitly accepts the no-flag, rollback-only-via-redeploy design for the pilot's risk profile, in writing (§7). Silence/deploying without this decision is not acceptable — this is precisely what R-061 exists to prevent. |
