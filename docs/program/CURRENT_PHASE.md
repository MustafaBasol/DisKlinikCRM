...
Son güncelleme: 2026-07-19 (F0-010 — Queue and Transactional Outbox Proof-of-Concept Design tamamlandı; ayrıca F0-009'un kendi taban-çizgisi commit'inde hâlâ `PR_OPEN` yazdığını tespit etti — `gh pr view 177` ile `MERGED` olarak düzeltildi, bkz. backlog F0-009 satırı.)
...
**F0-010 — Queue and Transactional Outbox Proof-of-Concept Design** → `PR_OPEN` — [PR #179](https://github.com/MustafaBasol/DisKlinikCRM/pull/179) açıldı, base `main`, dış inceleme bekliyor. İzole worktree `D:\Mustafa\Siteler\DisKlinikCRM-worktrees\f0-010-queue-outbox-poc`, branch `docs/f0-010-queue-outbox-poc-design`, base `origin/main` @ `23db9c3f1c93a564e094ae5f13be71ca3daa81ce` (F0-009'un kendi merge commit'i, PR #177). Depo-kanıtına dayalı PoC tasarım dokümanı ([../architecture/queue-outbox-poc-design.md](../architecture/queue-outbox-poc-design.md)), 18 iş akışı + 3 altyapı-primitifinin async envanteri ([../architecture/evidence/f0-010-async-flow-inventory.json](../architecture/evidence/f0-010-async-flow-inventory.json)), ve 25 deneylik PoC test matrisi ([../architecture/f0-010-poc-test-matrix.md](../architecture/f0-010-poc-test-matrix.md)) üretildi. Hiçbir uygulama/şema/migration/deployment dosyası değiştirilmedi; F0-007 mimari dondurma sınırı tam korunmuştur — bu görev yalnızca tasarımdır, OutboxEvent şeması/migration'ı, kuyruk bağımlılığı kurulumu, Redis deployment, BullMQ/Kafka adoption'ı yetkilendirmez. ADR-006/ADR-007 `DEFERRED` → `NEEDS_POC` (bu görevin kendi belgesel kaydı, dış inceleme bekliyor). `MERGED`/`DEPLOYED`/`PRODUCTION_VERIFIED` dış teyit gerektirir.

**Görev sırasında bulunan öz-referans gecikmesi:** bu görevin taban-çizgisi commit'i ([PR #177](https://github.com/MustafaBasol/DisKlinikCRM/pull/177)'nin kendi merge commit'i, `23db9c3f1c93a564e094ae5f13be71ca3daa81ce`) tracker/faz-dokümanı içeriğinde hâlâ F0-009'u `PR_OPEN` olarak gösteriyordu — F0-002…F0-008'de tekrarlanan aynı desen. `gh pr view 177` ile `MERGED` (`mergedAt: 2026-07-19T15:53:24Z`) olarak bağımsız doğrulandı ve düzeltildi.

## Sonraki görev

**[PR #179](https://github.com/MustafaBasol/DisKlinikCRM/pull/179)'un dış incelemesi ve merge kararı.** F0-009-S1 (SecurityIncident raw-SQL tenant-sahiplik doğrulaması ve sertleştirmesi) bu görevle paralel olarak, ayrı bir worktree/branch'te yetkilendirilmişti; içeriği bu görev tarafından okunmadı/değiştirilmedi, ancak [PR #178](https://github.com/MustafaBasol/DisKlinikCRM/pull/178) olarak `MERGED` olduğu (yalnızca meta veri düzeyinde) gözlemlendi. F0-011 ve F0-012 bağımlılık-hazır ancak başlatılmaları ayrı bir kullanıcı kararı gerektirir. F0-002…F0-009 hepsi `main`'e merge edilmiş. KVKK-HIGH-007 devamı ([PR #175](https://github.com/MustafaBasol/DisKlinikCRM/pull/175)) `MERGED` (commit `1da9586995b625624b7385c14e70ba6a322def73`) ama production-doğrulanmamış — programın kalan birincil blokajı production doğrulaması/backfill/flag-etkinleştirme ve dış "taban çizgisi kararlı" beyanına daralmıştır (bkz. [KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md](KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md) §5 koşul 3-5). Ayrıca bu görev sırasında birincil çalışma ağacında (`D:\Mustafa\Siteler\DisKlinikCRM`), salt-okunur olarak, yeni bir dal (`feature/kvkk-high008-legacy-consent-correction`, 16 modified + 9 untracked path, içerik incelenmedi) gözlemlendi — bir sonraki görev bunun kapsamını F0-007'nin KVKK-HIGH-007 için yaptığı gibi belgelemelidir.

**F0-011-P1 update (2026-07-19):** bu belgeleme görevi tamamlandı. Fresh capture: KVKK-HIGH-008 dalının HEAD'i artık `73ff3e90...`, birincil ağacın kirli (uncommitted) durumu 16+9 değil **4 modified + 1 untracked** yoldur (önceki rakam bu dalın hiçbir commit'i yokken alınmıştı ve artık bayattır). Dal zaten [PR #180](https://github.com/MustafaBasol/DisKlinikCRM/pull/180) olarak açık (`OPEN`, 0 inceleme, CI çalışmadı). Tam bulgular: [evidence/F0-011-P1_KVKK_HIGH008_ACTIVE_WORK_BASELINE.md](evidence/F0-011-P1_KVKK_HIGH008_ACTIVE_WORK_BASELINE.md), [KVKK_HIGH008_FREEZE_BOUNDARY.md](KVKK_HIGH008_FREEZE_BOUNDARY.md). Sonuç: dar, katmalı (additive-only), tenant-scoped, transactional-audited bir düzeltme iş akışı. **Düzeltme (2026-07-19, aynı gün):** bu madde başlangıçta çalışma zamanı kodunu, F0-011-P1'in belgeleme-only yetkisini aşan genel bir yetkilendirme kategorisiyle, "kod-inceleme/merge-hazırlığı için" `ALLOWED_NOW_NARROW_CORRECTIVE_FIX` olarak sınıflandırmıştı. Artık `IMPLEMENTATION_ACCEPTANCE_UNVERIFIED` ve `MERGE_REQUIRES_EXTERNAL_ARCHITECTURE_REVIEW` olarak sınıflandırılıyor — bu bir kabul değil, salt-okunur bir bulgudur; merge bu veya herhangi bir F0-011-P1 belgesi tarafından yetkilendirilmemiştir. Migration'ın production'a uygulanması ve iş akışının production'a deploy'u `MIGRATION_APPLICATION_BLOCKED` / `DEPLOYMENT_BLOCKED` olarak kalır (KVKK-HIGH-007'nin kendi production-migration durumu hâlâ teyit edilmedi) — ayrıca iş akışının feature flag/kill switch içermemesi açık bir karar gerektiren bir bulgu olarak kaydedildi. Bu görev KVKK-HIGH-008'i kabul etmedi, merge/test/deploy onaylamadı; PR #180 merge-güvenli olarak tanımlanmamıştır; ilgili dokümanların non-authorization ifadelerine bkz. Ana F0-011 (Object Storage and Backup Migration Design) görevi bu paralel görev tarafından incelenmedi/değiştirilmedi.
