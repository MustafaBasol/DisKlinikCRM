...
**F0-011-P1 update (2026-07-19):** bu belgeleme görevi tamamlandı. Fresh capture: KVKK-HIGH-008 dalının HEAD'i artık `73ff3e90...`, birincil ağacın kirli (uncommitted) durumu 16+9 değil **4 modified + 1 untracked** yoldur (önceki rakam bu dalın hiçbir commit'i yokken alınmıştı ve artık bayattır). Dal zaten [PR #180](https://github.com/MustafaBasol/DisKlinikCRM/pull/180) olarak açık (`OPEN`, 0 inceleme, CI çalışmadı). Tam bulgular: [evidence/F0-011-P1_KVKK_HIGH008_ACTIVE_WORK_BASELINE.md](evidence/F0-011-P1_KVKK_HIGH008_ACTIVE_WORK_BASELINE.md), [KVKK_HIGH008_FREEZE_BOUNDARY.md](KVKK_HIGH008_FREEZE_BOUNDARY.md). Sonuç: dar, katmalı (additive-only), tenant-scoped, transactional-audited bir düzeltme iş akışı; kod-inceleme/merge-hazırlığı için `ALLOWED_NOW_NARROW_CORRECTIVE_FIX`, ancak migration'ın production'a uygulanması ve iş akışının production'a deploy'u `BLOCKED_UNTIL_PR175_PRODUCTION_MIGRATION_VERIFIED` (KVKK-HIGH-007'nin kendi production-migration durumu hâlâ teyit edilmedi) — ayrıca iş akışının feature flag/kill switch içermemesi açık bir karar gerektiren bir bulgu olarak kaydedildi. Bu görev KVKK-HIGH-008'i kabul etmedi, merge/test/deploy onaylamadı; ilgili dokümanların non-authorization ifadelerine bkz. Ana F0-011 (Object Storage and Backup Migration Design) görevi bu paralel görev tarafından incelenmedi/değiştirilmedi.
