...
**F0-008 — ADR Review and Enterprise Foundation Decision Set** → `PR_OPEN` — [PR #176](https://github.com/MustafaBasol/DisKlinikCRM/pull/176) açıldı, base `main`, dış inceleme bekliyor. İzole worktree `D:\Mustafa\Siteler\DisKlinikCRM-worktrees\f0-008-adr-foundation-review`, branch `docs/f0-008-adr-foundation-decisions`, base `origin/main` @ `7cf7a827277779091b9e34e726eebccd39f624ae` (F0-007'nin kendi merge commit'i, PR #174). 17 ADR (ADR-001…017) depo kanıtına karşı gözden geçirildi: 2 `ACCEPTED`, 8 `ACCEPTED_WITH_CONDITIONS`, 5 `DEFERRED`, 2 `NEEDS_POC` — bkz. [ARCHITECTURE_DECISIONS.md](ARCHITECTURE_DECISIONS.md), [../architecture/adr-foundation-review.md](../architecture/adr-foundation-review.md), [../architecture/enterprise-foundation-decision-set.md](../architecture/enterprise-foundation-decision-set.md). Hiçbir ADR kabulü uygulama/şema/migration/deployment değişikliği yetkilendirmez; F0-007 mimari dondurma sınırı tam korunmuştur. `MERGED`/`DEPLOYED`/`PRODUCTION_VERIFIED` dış teyit gerektirir.

**Görev sırasında bulunan öz-referans gecikmesi:** bu görevin taban-çizgisi commit'i ([PR #174](https://github.com/MustafaBasol/DisKlinikCRM/pull/174)'ün kendi merge commit'i, `7cf7a827277779091b9e34e726eebccd39f624ae`) tracker/faz-dokümanı içeriğinde hâlâ F0-007'yi `PR_OPEN` olarak gösteriyordu — F0-002…F0-006'da tekrarlanan aynı desen. `git log`/`gh pr view 174` ile `MERGED` (`mergedAt: 2026-07-19T13:44:32Z`) olarak bağımsız doğrulandı ve düzeltildi.

## Sonraki görev

**[PR #176](https://github.com/MustafaBasol/DisKlinikCRM/pull/176)'nın dış incelemesi ve merge kararı.** Bu incelemeden sonraki kesin sıradaki görev: **F0-009 — RLS, Prisma, and PgBouncer Proof-of-Concept Design** veya **F0-010 — Queue and Transactional Outbox Proof-of-Concept Design** (ikisi de bağımlılık-hazır: F0-004 + F0-008, ikisi de artık karşılanmış; ikisi de yalnızca tasarım kapsamındadır). F0-011 ve F0-012 de bağımlılık-hazır ancak F0-011'in başlatılması ayrı bir kullanıcı kararı gerektirir. F0-002…F0-007 hepsi `main`'e merge edilmiş; F0-008 `AGENT_COMPLETED`. Programın kalan birincil blokajı değişmedi: KVKK-HIGH-007 devamı ([PR #175](https://github.com/MustafaBasol/DisKlinikCRM/pull/175)) hâlâ `OPEN`.
