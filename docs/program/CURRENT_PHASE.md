...
**F0-008 — ADR Review and Enterprise Foundation Decision Set** → `PR_OPEN` — [PR #176](https://github.com/MustafaBasol/DisKlinikCRM/pull/176) açıldı, base `main`, dış inceleme bekliyor. İzole worktree `D:\Mustafa\Siteler\DisKlinikCRM-worktrees\f0-008-adr-foundation-review`, branch `docs/f0-008-adr-foundation-decisions`, orijinal base `origin/main` @ `7cf7a827277779091b9e34e726eebccd39f624ae` (F0-007'nin kendi merge commit'i, PR #174), **correction-pass base** `origin/main` @ `1da9586995b625624b7385c14e70ba6a322def73` (PR #175, merged; merge-forward commit `e8255b2bffe55c3ca19040824320564cbcc48281`). 17 ADR (ADR-001…017) depo kanıtına karşı gözden geçirildi: 2 `ACCEPTED`, 8 `ACCEPTED_WITH_CONDITIONS`, 5 `DEFERRED`, 2 `NEEDS_POC` — bkz. [ARCHITECTURE_DECISIONS.md](ARCHITECTURE_DECISIONS.md), [../architecture/adr-foundation-review.md](../architecture/adr-foundation-review.md), [../architecture/enterprise-foundation-decision-set.md](../architecture/enterprise-foundation-decision-set.md). Hiçbir ADR kabulü uygulama/şema/migration/deployment değişikliği yetkilendirmez; F0-007 mimari dondurma sınırı tam korunmuştur. Correction pass, PR #175'in merge edilmiş olduğunu doğruladı ve ilgili belgeleri güncelledi; ADR sınıflandırmaları değişmedi. `MERGED`/`DEPLOYED`/`PRODUCTION_VERIFIED` dış teyit gerektirir.
...
**[PR #176](https://github.com/MustafaBasol/DisKlinikCRM/pull/176)'nın dış incelemesi ve merge kararı.** Bu incelemeden sonraki kesin sıradaki görev: **F0-009 — RLS, Prisma, and PgBouncer Proof-of-Concept Design** veya **F0-010 — Queue and Transactional Outbox Proof-of-Concept Design** (ikisi de bağımlılık-hazır: F0-004 + F0-008, ikisi de artık karşılanmış; ikisi de yalnızca tasarım kapsamındadır — **uygulama** hâlâ [KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md](KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md) §3'e göre bloklu). F0-011 ve F0-012 de bağımlılık-hazır ancak F0-011'in başlatılması ayrı bir kullanıcı kararı gerektirir. F0-002…F0-007 hepsi `main`'e merge edilmiş; F0-008 `PR_OPEN`. **Correction pass (2026-07-19):** KVKK-HIGH-007 devamı ([PR #175](https://github.com/MustafaBasol/DisKlinikCRM/pull/175)) artık `MERGED` (commit `1da9586995b625624b7385c14e70ba6a322def73`) — programın kalan birincil blokajı bu nedenle production doğrulaması/backfill/flag-etkinleştirme ve dış "taban çizgisi kararlı" beyanına daralmıştır (bkz. [KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md](KVKK_ARCHITECTURE_FREEZE_BOUNDARY.md) §5 koşul 3-5), "PR açık/merge edilmedi" değil.
