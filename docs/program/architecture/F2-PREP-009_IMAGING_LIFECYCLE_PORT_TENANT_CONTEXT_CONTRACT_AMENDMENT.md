...
**A raw JWT/session-default `clinicId` is not, by itself, sufficient authorization.** In this codebase, `req.user.clinicId` (and the JWT-derived default clinic it represents) is a UI/session default clinic *selection*, not a proof that the current principal is still authorized to act on that clinic's data — a JWT claim can be stale relative to the user's current `allowedClinicIds`/`canAccessAllClinics` grants, and nothing about possessing a JWT with a `clinicId` claim proves the accessible-clinic check has been (re-)run. No existing accepted route in this codebase applies `req.user.clinicId` (or any caller-supplied `clinicId`) directly to a Prisma `where` clause without first passing it through one of this codebase's own scope-validation mechanisms, and this port must not be the first exception:
...
- **Principal-initiated (interactive):** a request-scoped call where `clinicId` is resolved via one of §3's validated mechanisms — `resolveEffectiveClinicId`, an equivalent explicit scope-validation call, or a record lookup whose own query was already scoped to `organizationId` + `allowedClinicIds` (e.g. `patientPrivacy.ts`'s `resolvePatient()`, verified in §8a for this port's three intended callers) — **never** the unvalidated `req.user.clinicId`/JWT claim taken as-is. Every existing accepted route already performs this validation step before touching tenant-scoped data; this amendment requires that same validated value, not a new or weaker one, to be threaded into the port.
...
- **Finding 2 (signature drift) — `VERIFIED_ON_PRIOR_HEAD` / `CORRECTED_ON_CURRENT_PR304_HEAD`.** The optional `fileExistsForTest` parameter on `checkImageStorageExists` no longer exists on the current head. The current exported signature is exactly `checkImageStorageExists(imageId: string): Promise<boolean>` — a one-argument function, matching the accepted F2-CC-14 arity. Test-time storage-failure injection is now provided by a separate, distinctly-named, module-private-state export, `__setImagingStorageExistenceCheckerForTest(override)`, which is not a parameter of `checkImageStorageExists` itself. **This finding is not erased** — it was real and correctly identified against the head it was reviewed against — but it no longer describes the current PR #304 head and must not be cited as an open implementation defect against `main`-bound work going forward.
