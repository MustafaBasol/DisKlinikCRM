...
2. **A bound-instance API hides the authorization boundary at the call site.** With Option A, every call to `markStorageMissing(clinicId, imageId)` is self-evidently scoped by reading the call; a reviewer or future maintainer sees the tenant argument in every diff that adds a call. With Option B, a call site reads `port.markStorageMissing(imageId)` — indistinguishable, at the point of use, from today's unsafe shape. The scoping information is one indirection away, in whatever code constructed `port`. This program's own convention (root `AGENTS.md`-adjacent practice — this is the only `AGENTS.md` in the repository; there is no `server/AGENTS.md` — and every accepted contract to date) favors explicit-at-call-site over implicit-via-closure specifically because it makes a missing/wrong scope visible in code review.
...
- Every mutation (`markStorageMissing`, `redactForAnonymization`) re-applies the **same full predicate as the read path** — `{ id: imageId, clinicId, study: { clinicId } }` — in its own `update`/`updateMany` call. It must never re-scope the mutation by `{ id: imageId, clinicId }` alone: dropping the `ImagingStudy.clinicId` half of the predicate on the write side would silently re-open the denormalization-mismatch gap for mutations even though the read path already closes it, and would leave the read and write paths enforcing two different effective guarantees for what must be one property. Never relying on a read-then-trust-the-earlier-read pattern across two separate queries.
- **Prisma capability, verified against this repository (2026-08-03), not assumed:** `server/package.json` pins `prisma`/`@prisma/client` at `7.8.0`. The generated client's `ImagingImageWhereUniqueInput` (`server/node_modules/.prisma/client/index.d.ts`) is `Prisma.AtLeast<{ id?, clinicId?, studyId?, ..., study?: XOR<ImagingStudyScalarRelationFilter, ImagingStudyWhereInput>, ... }, "id">` — this repository's generated client supports Prisma's extended-`whereUnique` combination of a unique field (`id`) with additional non-unique filters, including the `study: { clinicId }` relation filter, on a **single-record** `update`/`delete`, not only on `updateMany`. The exact predicate `{ id: imageId, clinicId, study: { clinicId } }` is therefore directly usable, unmodified, on either a singular `update`/`delete` or an `updateMany` call against `ImagingImage` in this codebase — `updateMany`'s own `ImagingImageWhereInput` (same file) independently supports the identical relation filter with no uniqueness constraint at all. No reduced or alternate write-side predicate is required to stay within Prisma's supported syntax; the read and write predicates can and must be identical.
