...
| Do filesystem backups exist? | No evidence found of any file-tree backup script/job/cron unit anywhere in the repository or `PRODUCTION_TOPOLOGY.md` | `VERIFIED_PRODUCTION_OBSERVED` (absence) |
...
| No-secondary-copy model | Current de facto state (single local disk, no backup of file bytes at all) — explicitly the weakest option and the gap this document flags most strongly (no file-tree backup was found anywhere) |
