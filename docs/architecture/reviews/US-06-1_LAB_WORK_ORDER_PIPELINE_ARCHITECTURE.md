...
This must be a **new scheduled job**, not a request-time computation like the current overdue notification (§2.8), because "48 hours before `expectedReturnDate`" requires evaluation independent of any user loading a page.

- **Job frequency**: hourly is sufficient given a 48-hour window (matches the granularity of `expectedReturnDate`, which is a date not a precise timestamp in current usage) — reuse the existing `node-cron` + `withJobLock` pattern from `reminders.ts` (§2.8) for multi-replica safety.
- **Clinic timezone**: must use `getClinicOperatingPreferences` (already used by `notifications.ts`) to compute "48 hours before, in clinic-local time" rather than server/UTC time — reusing existing infrastructure, not inventing new timezone handling.
- **Eligibility criteria**: `status IN PRE_RECEIPT_STATUSES` (reuse the existing constant, §2.3) AND `expectedReturnDate` is between `now` and `now + 48h` AND not already `cancelled`/`deletedAt`.
...
- **Idempotency key**: reuse `Notification.externalId`'s atomic-upsert pattern (§2.8), e.g. `lab-risk-48h-${orderId}` — distinct from the existing `lab-overdue-${id}` key so the two notification types don't collide, and the upsert-never-resets-`isRead` behavior is inherited for free.
- **Duplicate prevention / re-notification policy**: because the key is per-order (not per-day), a single upsert naturally fires once per order per 48h-window entry — no re-notification unless the order re-enters `PRE_RECEIPT_STATUSES` (e.g. via a rework loop-back) with a new `expectedReturnDate`, at which point a *new* eligibility window naturally begins. Recommend explicitly **not** re-firing daily inside the same window, to avoid alert fatigue — this matches the "notify once per meaningful state change" philosophy already used by the `Setting`-keyed dedup pattern in `reminders.ts`.
- **Appointment rescheduling**: if §9.1's linkage exists, a change to `expectedReturnDate` or a change to a linked appointment's time should be evaluated by the job on its next run — no separate reactive trigger needed given the hourly cadence, keeping the design simple.
- **Lab-order status update / cancellation**: as soon as status leaves `PRE_RECEIPT_STATUSES` (received, or cancelled), the order stops being eligible on the next run — no explicit "cancel the notification" step needed since eligibility is re-evaluated each run rather than being a one-shot fire-and-forget.
- **After order receipt**: the risk notification becomes moot; nothing further needs to happen to it (it simply stops being generated; existing unread notifications are left as historical record, consistent with how `Notification` has no TTL field today, §2.8).
...
- New 48h risk-warning job (§9.2), deployed and monitored *after* Phases A–D are stable — it depends on `expectedReturnDate` and status data that already exist, so it has no hard schema dependency on Phases A–D beyond wanting `LabWorkOrderCostHistory`/appointment links to exist first if the notification content is meant to reference them.
