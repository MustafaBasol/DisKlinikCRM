...
## 0-A. Evidence supersession notice (added 2026-08-21 by G1-TECH-PREFLIGHT-001)

**This document's §2 checks 3, 4, 6 and 11, and its §5 go/no-go summary, were written against baseline `3b4ec9d` and are partly stale.** They are preserved verbatim below — this package does not rewrite its own history — but they must not be relied on without reading the reconciliation first:

> [../../program/evidence/G1-TECH-PREFLIGHT-001_FIRST_CLINIC_TECHNICAL_PREFLIGHT.md](../../program/evidence/G1-TECH-PREFLIGHT-001_FIRST_CLINIC_TECHNICAL_PREFLIGHT.md) — fresh repository evidence at `origin/main` @ `ef90dfb` and fresh read-only production evidence from `disklinik-prod-01` dated 2026-08-21.

Summary of what changed, per item (detail and evidence in the linked document):

| §2 check | Was | Is now |
|---|---|---|
| 3 — Backup evidence | "~11 hour" same-host `pg_dump` interval | pgBackRest 2.50 with **repo1 (local) + repo2 (off-host, encrypted SFTP)**, both cron-scheduled, `pgbackrest check` passing on both |
| 4 — Restore-test evidence | "**no evidence of an actual run exists** … it blocks the first clinic's onboarding" | **R-032 is `CLOSED`.** Three restore drills passed; the newest restored from the off-host repo with RPO 2 min / RTO 25 s, 80/80 migration parity, and application + tenant-isolation smoke both passing. This item is no longer a blocker. |
| 6 — User/role creation | "**Known gap** — no bootstrap/invite flow for a brand-new tenant's first user" | **Gap confirmed still real at `ef90dfb`, then closed additively**: `POST /api/platform/clinics/:id/owner` creates the first OWNER for a **zero-user** clinic only, audited and atomic. **Implemented and tested; not yet merged or deployed** — until it is, treat this check as still open, and do **not** substitute a manual DB mutation for it. |
| 11 — G1-blocking risk status | R-046, R-061, R-029, R-030, R-031, R-032 all treated as open | **R-061, R-031, R-032 are `CLOSED`; R-062 is `MITIGATED`.** **R-046 remains `OPEN` and is the one named G1 blocker still genuinely outstanding.** R-030-DB is technically complete and held only by the KVKK Workload-B counsel gate; R-029/R-030-FILES (off-host backup of the upload tree) remains technically open. |
| §5 go/no-go | "R-046 and R-061 are `OPEN` and R-032 has no recorded execution" | Two of those three named blockers (R-061, R-032) are closed. **The go/no-go decision still cannot be `GO`** — but now because of R-046, the un-deployed owner bootstrap, Platform Admin MFA enrolment, R-030-FILES, and the unresolved per-clinic legal determination. |

Nothing in this notice approves G1, closes the legal gate, or authorizes onboarding any clinic. `G1` remains `NOT_APPROVED` per [../../program/RELEASE_GATES.md](../../program/RELEASE_GATES.md).
...
| 3 | **Backup evidence** ⚠️ *superseded — see §0-A* | Most recent `pg_dump` backup timestamp is within the currently observed backup interval (documented as ~11 hours as of this baseline); backup existence and freshness are confirmed by a fresh check, not assumed | [../../program/RISK_REGISTER.md](../../program/RISK_REGISTER.md) R-030; [../../program/LAUNCH_GATES.md](../../program/LAUNCH_GATES.md) §2.E |
| 4 | **Restore-test evidence** ⚠️ *superseded — R-032 is now `CLOSED`, see §0-A* | At least one successful restore rehearsal against a disposable environment is on record, with dated output. As of this baseline, [../../program/RISK_REGISTER.md](../../program/RISK_REGISTER.md) R-032 records the capability (`runRestoreTest()`, `server/src/services/backupService.ts`) exists but **no evidence of an actual run exists** — this is a **currently unmet** pre-onboarding check, not a formality; it blocks the first clinic's onboarding until closed (see §5 below) | [../../program/RISK_REGISTER.md](../../program/RISK_REGISTER.md) R-032; [../../architecture/f0-011-storage-backup-test-matrix.md](../../architecture/f0-011-storage-backup-test-matrix.md) Experiments 25-27 |
...
3. **Create the first user(s).** Follow whatever first-user bootstrap procedure the deployed environment actually supports (§2 check 6) and record it verbatim, since no such procedure is documented in this repository as of this baseline. Create the OWNER, then any additional named roles the clinic needs, via the authenticated `POST /api/users` path.
   > **Update (2026-08-21, G1-TECH-PREFLIGHT-001 — see §0-A):** the intended procedure is now `POST /api/platform/clinics/:id/owner`, which creates the first OWNER for a **zero-user** clinic only, inside one transaction with its `platform_clinic.owner_bootstrapped` audit row, and emails the new owner a one-time link to set their own password (the operator never chooses or learns it). **This route is not yet merged or deployed** — until a fresh check confirms it is live on the deployed release SHA, this step is still open, and a manual/raw-SQL user insert is **not** an acceptable substitute for it.
