...
**Tests added:** `server/src/tests/kvkkAttachmentImagingLifecycle.test.ts` — 55 unit tests (storage-key safety, real local-disk `fileExists`/`statFile`, export-token validation incl. tenant isolation, export-cleanup job dependency-injected behavior, anonymization redaction semantics incl. legal-hold skip and partial-failure counting, deletion-review dry-run inventory, orphan classification, migration-cleanliness + migration-shape regression checks, live-delete-removal/header-token source scans, atomic one-time claim, export bounds/reason-code checks, cleanup kill-switch, legal-hold reason/audit checks, i18n key parity). Registered as `npm run test:kvkk-lifecycle`, wired into the `test` chain. Also added `server/scripts/verify-export-archive-lifecycle.ts` — a manual (not wired into `npm test`) disposable-DB script exercising the real `PatientPrivacyExportArchive` reserve/complete/claim/fail lifecycle against a real Postgres database with real fixtures.

**Exact validation commands and results (this session, 2026-07-15, migration-drift follow-up pass):**
- `npx prisma validate` → `The schema at prisma\schema.prisma is valid`
...
- `information_schema.columns` inspected directly against the disposable database → confirmed `status`/nullability/index shape matches `schema.prisma` exactly
- `npx prisma migrate diff --from-config-datasource prisma/schema.prisma --to-schema prisma/schema.prisma --script` against the disposable database → zero remaining diff for `PatientPrivacyExportArchive`/`PatientAttachment`/`ImagingStudy`/`ImagingImage`; remaining diff lines are pre-existing, unrelated repo-wide drift on tables this branch never touches
- `DATABASE_URL=<disposable> npx tsx scripts/verify-export-archive-lifecycle.ts` → **6 passed, 0 failed** (real reserve/complete/claim/fail lifecycle against real Postgres)
- `npm run typecheck` (server) → 0 errors
- `npx tsx src/tests/kvkkAttachmentImagingLifecycle.test.ts` → **55 passed, 0 failed**
