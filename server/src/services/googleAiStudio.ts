...
export const extractAssistantInputWithGoogleAi = async (
  ...
  const prompt = [
    ...
    '- afterTime should be normalized like 11:00 when the user says 11 den sonra / 11 sonrası / after 11. Also recognize Turkish word numbers: "iki buçuk ve sonrası" → afterTime "02:30", "öğleden sonra iki buçuk ve sonrası" → afterTime "14:30", "üç sonrası" → afterTime "15:00" (when context is afternoon/PM).',
    '- exactTime also handles Turkish word numbers: "öğleden sonra iki buçuk" → exactTime "14:30", "sabah sekiz" → exactTime "08:00", "üç" (with PM context) → exactTime "15:00".',
