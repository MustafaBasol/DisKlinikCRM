...
import { getExternalCalendarConnectionRecord } from './externalCalendarConnectionService.js';
...
async function getIntegrationEnabledAndProvider(
  clinicId: string,
): Promise<{ enabled: boolean; provider: string } | null> {
...
async function finalizeFailure(
  ...
  await prisma.externalCalendarAppointmentLink.update({
    ...
    data: {
      status,
      lastError: exhausted && classification.retryable
  ...
  // Staff-visible operational failure — sanitized message only, never a raw
  // provider payload, secret, or patient field.
  console.error('[external-calendar-outbound-sync] sync attempt failed', {
    linkId: link.id,
...
export async function attemptExternalCalendarSync(
  ...
  try {
    ...
    await sendConfirmationForConvertedAppointment(appointment.id, sendConfirmation).catch((err) => {
      console.error('[external-calendar-outbound-sync] post-sync confirmation notification failed', err);
    });
