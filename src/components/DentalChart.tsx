...
const Tooth: React.FC<ToothProps> = ({ fdi, record, onClick }) => {
...
const JawRow: React.FC<JawRowProps> = ({ left, right, records, onTooth, label }) => (
  <div className="flex items-center gap-1">
    ...
    <div className="flex items-end gap-0.5 justify-end">
      {left.map((fdi) => (
        <Tooth key={fdi} fdi={fdi} record={records.get(fdi)} onClick={onTooth} />
      ))}
    </div>
...
// ── Main component ─────────────────────────────────────────────────────
const DentalChart: React.FC<Props> = ({ patientId, readOnly = false }) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const res = await dentalChartService.getAll(patientId);
        const map = new Map<number, ToothRecord>();
        for (const r of res.data) map.set(r.toothFdi, r);
        setRecords(map);
      } finally {
        setLoading(false);
      }
          ...
          <div className="divide-y dark:divide-gray-700">
            {[...records.values()].map((r) => {
              ...
              return (
                ...
                >
                  <span
                    className={`w-8 h-8 rounded-lg border-2 flex items-center justify-center text-xs font-bold flex-shrink-0 ${cfg.bg} ${cfg.text} ${cfg.border} ${cfg.darkBg}`}
                  >
                    {r.toothFdi % 10}
                  </span>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-xs px-1.5 py-0.5 rounded-full ${cfg.bg} ${cfg.text} ${cfg.darkBg}`}>
                        {cfg.label}
                      </span>
                    </div>
                    {r.note && (
                      <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{r.note}</p>
                    )}
                  </div>
                  {r.createdBy && (
